...
class ExportedService(object):
    ...
    def get_exported_services(self):
        manifest = self.xml_parser.parsed_manifest()
        paths = ['//service[not(@android:permission)]/intent-filter/../@android:name',
                 '//service[not(@android:permission) and (@android:exported="true")]/@android:name']
