...
public class zzjz
{
  ...
  public static String zza(Calendar paramCalendar)
  {
    ...
    return ((String)localObject).replace("+0000", zzVs[0]);
  }
  ...
  public static Calendar zzbK(String paramString)
  {
    ...
    Object localObject2 = zzbL(paramString);
    if (TextUtils.isEmpty((CharSequence)localObject2))
    {
      zzQW.zzb("Invalid date format", new Object[0]);
      return null;
    }
    String str = zzbM(paramString);
    ...
    for (paramString = "yyyyMMdd'T'HHmmss";; paramString = zzVt)
    {
      ...
      {
        label127:
        zzQW.zzb("Error parsing string: %s", new Object[] { paramString.getMessage() });
      }
    }
    return null;
  }
  
  private static String zzbL(String paramString)
  {
    if (TextUtils.isEmpty(paramString))
    {
      zzQW.zzb("Input string is empty or null", new Object[0]);
    ...
    {
      zzQW.zze("Error extracting the date: %s", new Object[] { paramString.getMessage() });
    }
    return null;
  }
  
  private static String zzbM(String paramString)
  {
    if (TextUtils.isEmpty(paramString)) {
    ...
    do
    {
      do
      {
        ...
        {
          zzQW.zzb("Error extracting the time substring: %s", new Object[] { paramString.getMessage() });
          return null;
        }
      ...
      } while (!zzbN(paramString));
      return paramString.replaceAll("([\\+\\-]\\d\\d):(\\d\\d)", "$1$2");
    } while (paramString.length() != "HHmmss".length() + zzVs[0].length());
    return paramString.substring(0, paramString.length() - 1) + "+0000";
  }
  
  private static boolean zzbN(String paramString)
  {
    int i = paramString.length();
    int j = "HHmmss".length();
    return (i == zzVs[1].length() + j) || (i == zzVs[2].length() + j) || (i == j + zzVs[3].length());
  }
}
