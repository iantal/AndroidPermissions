  ...
  extends Thing
{
    ...
    extends Thing.Builder
  {
    ...
    public Action build()
    {
      ...
      Bundle localBundle = (Bundle)this.zzNW.getParcelable("object");
      zzu.zzb(localBundle.get("name"), "Must call setObject() with a valid name. Example: setObject(new Thing.Builder().setName(name).setUrl(url))");
      zzu.zzb(localBundle.get("url"), "Must call setObject() with a valid app URI. Example: setObject(new Thing.Builder().setName(name).setUrl(url))");
