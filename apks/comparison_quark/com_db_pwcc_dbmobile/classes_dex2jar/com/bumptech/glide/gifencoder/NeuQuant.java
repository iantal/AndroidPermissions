package com.bumptech.glide.gifencoder;

class NeuQuant
{
  protected static final int alphabiasshift = 10;
  protected static final int alpharadbias = 262144;
  protected static final int alpharadbshift = 18;
  protected static final int beta = 64;
  protected static final int betagamma = 65536;
  protected static final int betashift = 10;
  protected static final int gamma = 1024;
  protected static final int gammashift = 10;
  protected static final int initalpha = 1024;
  protected static final int initrad = 32;
  protected static final int initradius = 2048;
  protected static final int intbias = 65536;
  protected static final int intbiasshift = 16;
  protected static final int maxnetpos = 255;
  protected static final int minpicturebytes = 1509;
  protected static final int ncycles = 100;
  protected static final int netbiasshift = 4;
  protected static final int netsize = 256;
  protected static final int prime1 = 499;
  protected static final int prime2 = 491;
  protected static final int prime3 = 487;
  protected static final int prime4 = 503;
  protected static final int radbias = 256;
  protected static final int radbiasshift = 8;
  protected static final int radiusbias = 64;
  protected static final int radiusbiasshift = 6;
  protected static final int radiusdec = 30;
  protected int alphadec;
  protected int[] bias = new int['Ā'];
  protected int[] freq = new int['Ā'];
  protected int lengthcount;
  protected int[] netindex = new int['Ā'];
  protected int[][] network;
  protected int[] radpower = new int[32];
  protected int samplefac;
  protected byte[] thepicture;
  
  public NeuQuant(byte[] paramArrayOfByte, int paramInt1, int paramInt2)
  {
    this.thepicture = paramArrayOfByte;
    this.lengthcount = paramInt1;
    this.samplefac = paramInt2;
    this.network = new int['Ā'][];
    for (int i = 0; i < 256; i++)
    {
      this.network[i] = new int[4];
      int[] arrayOfInt = this.network[i];
      int j = (i << 12) / 256;
      arrayOfInt[2] = j;
      arrayOfInt[1] = j;
      arrayOfInt[0] = j;
      this.freq[i] = 256;
      this.bias[i] = 0;
    }
  }
  
  protected void alterneigh(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5)
  {
    int i = paramInt2 - paramInt1;
    if (i < -1) {}
    for (int j = -1;; j = i)
    {
      int k = paramInt2 + paramInt1;
      if (k > 256) {
        k = 256;
      }
      int m = paramInt2 - 1;
      int n = paramInt2 + 1;
      int i1 = 1;
      int i2 = n;
      int i3 = m;
      for (;;)
      {
        int i4;
        int i5;
        int i6;
        int[] arrayOfInt5;
        if ((i2 < k) || (i3 > j))
        {
          int[] arrayOfInt1 = this.radpower;
          i4 = i1 + 1;
          i5 = arrayOfInt1[i1];
          if (i2 < k)
          {
            int[][] arrayOfInt4 = this.network;
            i6 = i2 + 1;
            arrayOfInt5 = arrayOfInt4[i2];
          }
        }
        try
        {
          arrayOfInt5[0] -= i5 * (arrayOfInt5[0] - paramInt3) / 262144;
          arrayOfInt5[1] -= i5 * (arrayOfInt5[1] - paramInt4) / 262144;
          arrayOfInt5[2] -= i5 * (arrayOfInt5[2] - paramInt5) / 262144;
          for (;;)
          {
            if (i3 > j)
            {
              int[][] arrayOfInt2 = this.network;
              int i7 = i3 - 1;
              int[] arrayOfInt3 = arrayOfInt2[i3];
              try
              {
                arrayOfInt3[0] -= i5 * (arrayOfInt3[0] - paramInt3) / 262144;
                arrayOfInt3[1] -= i5 * (arrayOfInt3[1] - paramInt4) / 262144;
                arrayOfInt3[2] -= i5 * (arrayOfInt3[2] - paramInt5) / 262144;
                i3 = i7;
                i2 = i6;
                i1 = i4;
              }
              catch (Exception localException1)
              {
                i3 = i7;
                i2 = i6;
                i1 = i4;
              }
              break;
            }
            i2 = i6;
            i1 = i4;
            break;
            i6 = i2;
          }
          return;
        }
        catch (Exception localException2)
        {
          for (;;) {}
        }
      }
    }
  }
  
  protected void altersingle(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5)
  {
    int[] arrayOfInt = this.network[paramInt2];
    arrayOfInt[0] -= paramInt1 * (arrayOfInt[0] - paramInt3) / 1024;
    arrayOfInt[1] -= paramInt1 * (arrayOfInt[1] - paramInt4) / 1024;
    arrayOfInt[2] -= paramInt1 * (arrayOfInt[2] - paramInt5) / 1024;
  }
  
  public byte[] colorMap()
  {
    byte[] arrayOfByte = new byte['̀'];
    int[] arrayOfInt = new int['Ā'];
    for (int i = 0; i < 256; i++) {
      arrayOfInt[this.network[i][3]] = i;
    }
    int j = 0;
    int k = 0;
    while (k < 256)
    {
      int m = arrayOfInt[k];
      int n = j + 1;
      arrayOfByte[j] = ((byte)this.network[m][0]);
      int i1 = n + 1;
      arrayOfByte[n] = ((byte)this.network[m][1]);
      arrayOfByte[i1] = ((byte)this.network[m][2]);
      k++;
      j = i1 + 1;
    }
    return arrayOfByte;
  }
  
  protected int contest(int paramInt1, int paramInt2, int paramInt3)
  {
    int i = Integer.MAX_VALUE;
    int j = -1;
    int k = i;
    int m = j;
    int n = 0;
    int i4;
    int i5;
    if (n < 256)
    {
      int[] arrayOfInt3 = this.network[n];
      int i1 = arrayOfInt3[0] - paramInt1;
      if (i1 < 0) {
        i1 = -i1;
      }
      int i2 = arrayOfInt3[1] - paramInt2;
      if (i2 < 0) {
        i2 = -i2;
      }
      int i3 = arrayOfInt3[2] - paramInt3;
      if (i3 < 0) {
        i3 = -i3;
      }
      i4 = i3 + (i1 + i2);
      if (i4 >= i) {
        break label267;
      }
      i5 = i4;
    }
    for (int i6 = n;; i6 = j)
    {
      int i7 = i4 - (this.bias[n] >> 12);
      if (i7 < k) {}
      for (int i8 = n;; i8 = m)
      {
        int i9 = this.freq[n] >> 10;
        int[] arrayOfInt4 = this.freq;
        arrayOfInt4[n] -= i9;
        int[] arrayOfInt5 = this.bias;
        arrayOfInt5[n] += (i9 << 10);
        n++;
        k = i7;
        m = i8;
        j = i6;
        i = i5;
        break;
        int[] arrayOfInt1 = this.freq;
        arrayOfInt1[j] = (64 + arrayOfInt1[j]);
        int[] arrayOfInt2 = this.bias;
        arrayOfInt2[j] -= 65536;
        return m;
        i7 = k;
      }
      label267:
      i5 = i;
    }
  }
  
  public void inxbuild()
  {
    int i = 0;
    int j = 0;
    int k = 0;
    int n;
    if (k < 256)
    {
      int[] arrayOfInt1 = this.network[k];
      n = arrayOfInt1[1];
      int i1 = k + 1;
      int i2 = k;
      while (i1 < 256)
      {
        int[] arrayOfInt3 = this.network[i1];
        if (arrayOfInt3[1] < n)
        {
          n = arrayOfInt3[1];
          i2 = i1;
        }
        i1++;
      }
      int[] arrayOfInt2 = this.network[i2];
      if (k != i2)
      {
        int i5 = arrayOfInt2[0];
        arrayOfInt2[0] = arrayOfInt1[0];
        arrayOfInt1[0] = i5;
        int i6 = arrayOfInt2[1];
        arrayOfInt2[1] = arrayOfInt1[1];
        arrayOfInt1[1] = i6;
        int i7 = arrayOfInt2[2];
        arrayOfInt2[2] = arrayOfInt1[2];
        arrayOfInt1[2] = i7;
        int i8 = arrayOfInt2[3];
        arrayOfInt2[3] = arrayOfInt1[3];
        arrayOfInt1[3] = i8;
      }
      if (n == i) {
        break label273;
      }
      this.netindex[i] = (j + k >> 1);
      for (int i4 = i + 1; i4 < n; i4++) {
        this.netindex[i4] = k;
      }
    }
    for (int i3 = k;; i3 = j)
    {
      k++;
      i = n;
      j = i3;
      break;
      this.netindex[i] = (j + 255 >> 1);
      for (int m = i + 1; m < 256; m++) {
        this.netindex[m] = 255;
      }
      return;
      label273:
      n = i;
    }
  }
  
  public void learn()
  {
    if (this.lengthcount < 1509) {
      this.samplefac = 1;
    }
    this.alphadec = (30 + (-1 + this.samplefac) / 3);
    byte[] arrayOfByte = this.thepicture;
    int i = this.lengthcount;
    int j = this.lengthcount / (3 * this.samplefac);
    int k = j / 100;
    for (int m = 0; m < 32; m++) {
      this.radpower[m] = (1024 * (256 * (1024 - m * m) / 1024));
    }
    int n;
    int i1;
    int i2;
    int i3;
    int i4;
    int i5;
    label135:
    int i10;
    if (this.lengthcount < 1509)
    {
      n = 3;
      i1 = 32;
      i2 = 0;
      i3 = 0;
      i4 = 2048;
      i5 = 1024;
      if (i3 >= j) {
        break label422;
      }
      int i6 = (0xFF & arrayOfByte[(i2 + 0)]) << 4;
      int i7 = (0xFF & arrayOfByte[(i2 + 1)]) << 4;
      int i8 = (0xFF & arrayOfByte[(i2 + 2)]) << 4;
      int i9 = contest(i6, i7, i8);
      altersingle(i5, i9, i6, i7, i8);
      if (i1 != 0) {
        alterneigh(i1, i9, i6, i7, i8);
      }
      i10 = i2 + n;
      if (i10 < i) {
        break label472;
      }
    }
    label422:
    label472:
    for (int i11 = i10 - this.lengthcount;; i11 = i10)
    {
      int i12 = i3 + 1;
      if (k == 0) {}
      for (int i13 = 1;; i13 = k)
      {
        if (i12 % i13 == 0)
        {
          int i14 = i5 - i5 / this.alphadec;
          int i15 = i4 - i4 / 30;
          int i16 = i15 >> 6;
          if (i16 <= 1) {
            i16 = 0;
          }
          int i17 = 0;
          for (;;)
          {
            if (i17 < i16)
            {
              this.radpower[i17] = (i14 * (256 * (i16 * i16 - i17 * i17) / (i16 * i16)));
              i17++;
              continue;
              if (this.lengthcount % 499 != 0)
              {
                n = 1497;
                break;
              }
              if (this.lengthcount % 491 != 0)
              {
                n = 1473;
                break;
              }
              if (this.lengthcount % 487 != 0)
              {
                n = 1461;
                break;
              }
              n = 1509;
              break;
              return;
            }
          }
          k = i13;
          i1 = i16;
          i2 = i11;
          i3 = i12;
          i4 = i15;
          i5 = i14;
          break label135;
        }
        k = i13;
        i2 = i11;
        i3 = i12;
        break label135;
      }
    }
  }
  
  public int map(int paramInt1, int paramInt2, int paramInt3)
  {
    int i = 1000;
    int j = this.netindex[paramInt2];
    int k = j - 1;
    int m = j;
    int n = -1;
    int i1 = k;
    int[] arrayOfInt2;
    int i8;
    int i2;
    if ((m < 256) || (i1 >= 0))
    {
      if (m >= 256) {
        break label320;
      }
      arrayOfInt2 = this.network[m];
      i8 = arrayOfInt2[1] - paramInt2;
      if (i8 >= i) {
        i2 = 256;
      }
    }
    for (;;)
    {
      if (i1 >= 0)
      {
        int[] arrayOfInt1 = this.network[i1];
        int i3 = paramInt2 - arrayOfInt1[1];
        if (i3 >= i)
        {
          m = i2;
          i1 = -1;
          break;
          i2 = m + 1;
          if (i8 < 0) {
            i8 = -i8;
          }
          int i9 = arrayOfInt2[0] - paramInt1;
          if (i9 < 0) {
            i9 = -i9;
          }
          int i10 = i9 + i8;
          if (i10 >= i) {
            continue;
          }
          int i11 = arrayOfInt2[2] - paramInt3;
          if (i11 < 0) {
            i11 = -i11;
          }
          int i12 = i11 + i10;
          if (i12 >= i) {
            continue;
          }
          int i13 = arrayOfInt2[3];
          i = i12;
          n = i13;
          continue;
        }
        i1--;
        if (i3 < 0) {
          i3 = -i3;
        }
        int i4 = arrayOfInt1[0] - paramInt1;
        if (i4 < 0) {
          i4 = -i4;
        }
        int i5 = i4 + i3;
        if (i5 < i)
        {
          int i6 = arrayOfInt1[2] - paramInt3;
          if (i6 < 0) {
            i6 = -i6;
          }
          int i7 = i6 + i5;
          if (i7 < i)
          {
            n = arrayOfInt1[3];
            m = i2;
            i = i7;
            break;
            return n;
          }
        }
      }
      m = i2;
      break;
      label320:
      i2 = m;
    }
  }
  
  public byte[] process()
  {
    learn();
    unbiasnet();
    inxbuild();
    return colorMap();
  }
  
  public void unbiasnet()
  {
    for (int i = 0; i < 256; i++)
    {
      int[] arrayOfInt1 = this.network[i];
      arrayOfInt1[0] >>= 4;
      int[] arrayOfInt2 = this.network[i];
      arrayOfInt2[1] >>= 4;
      int[] arrayOfInt3 = this.network[i];
      arrayOfInt3[2] >>= 4;
      this.network[i][3] = i;
    }
  }
}
