...
public final class LoggerFactory
{
  ...
  static Set<URL> findPossibleStaticLoggerBinderPathSet()
  {
    ...
    for (;;)
    {
      ...
      try
      {
        localClassLoader = LoggerFactory.class.getClassLoader();
      ...
      Enumeration localEnumeration = localClassLoader.getResources(STATIC_LOGGER_BINDER_PATH);
  ...
  public static Logger getLogger(Class<?> paramClass)
  {
    ...
    if (DETECT_LOGGER_NAME_MISMATCH)
    {
      ...
      if ((localClass != null) && (nonMatchingClasses(paramClass, localClass)))
      {
        ...
        arrayOfObject[0] = localLogger.getName();
        arrayOfObject[1] = localClass.getName();
        Util.report(String.format("Detected logger name mismatch. Given name: \"%s\"; computed name: \"%s\".", arrayOfObject));
  ...
  private static void replayEvents()
  {
    ...
    for (;;)
    {
      ...
      for (int k = j; localIterator.hasNext(); k++)
      {
        ...
        if (k == 0) {
          emitReplayOrSubstituionWarning(localSubstituteLoggingEvent, i);
        }
      }
