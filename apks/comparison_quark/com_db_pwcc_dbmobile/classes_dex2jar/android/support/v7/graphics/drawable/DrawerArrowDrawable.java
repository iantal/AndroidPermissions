  ...
  public void draw(Canvas paramCanvas)
  {
    ...
    default: 
      if (DrawableCompat.getLayoutDirection(this) == 1)
      {
        ...
        f7 = 180.0F;
        float f8 = lerp(f6, f7, this.mProgress);
        float f9 = (float)Math.round(f2 * Math.cos(f5));
        float f10 = (float)Math.round(f2 * Math.sin(f5));
        this.mPath.rewind();
        float f11 = lerp(this.mBarGap + this.mPaint.getStrokeWidth(), -this.mMaxCutForBarSize, this.mProgress);
        float f12 = -f3 / 2.0F;
        this.mPath.moveTo(f12 + f4, 0.0F);
        this.mPath.rLineTo(f3 - f4 * 2.0F, 0.0F);
        this.mPath.moveTo(f12, f11);
        this.mPath.rLineTo(f9, f10);
        this.mPath.moveTo(f12, -f11);
        this.mPath.rLineTo(f9, -f10);
        this.mPath.close();
        paramCanvas.save();
        float f13 = this.mPaint.getStrokeWidth();
        float f14 = 2 * ((int)(localRect.height() - 3.0F * f13 - 2.0F * this.mBarGap) / 4);
        float f15 = this.mBarGap;
        paramCanvas.translate(localRect.centerX(), f14 + (f15 + f13 * 1.5F));
        if (!this.mSpin) {
          break label478;
        }
