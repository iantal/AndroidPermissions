  ...
  private final ThreadUtil.MainThreadCallback<T> mMainThreadCallback = new ThreadUtil.MainThreadCallback()
  {
    ...
    public void addTile(int paramAnonymousInt, TileList.Tile<T> paramAnonymousTile)
    {
      ...
      for (;;)
      {
        ...
        int k = 0;
        while (k < AsyncListUtil.this.mMissingPositions.size())
        {
          int m = AsyncListUtil.this.mMissingPositions.keyAt(k);
          if ((paramAnonymousTile.mStartPosition <= m) && (m < i + j))
          {
            AsyncListUtil.this.mMissingPositions.removeAt(k);
            AsyncListUtil.this.mViewCallback.onItemLoaded(m);
          }
          else
          {
            k++;
