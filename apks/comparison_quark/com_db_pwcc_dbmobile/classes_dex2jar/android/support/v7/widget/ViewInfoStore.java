...
class ViewInfoStore {
     ...
     */
    private RecyclerView.ItemAnimator.ItemHolderInfo popFromLayoutStep(RecyclerView.ViewHolder viewHolder, int n2) {
        int n3 = this.mLayoutHolderMap.indexOfKey(viewHolder);
        ...
        int n4 = n2 & infoRecord.flags;
        itemHolderInfo = null;
        if (n4 == 0) return itemHolderInfo;
        infoRecord.flags &= ~ n2;
        if (n2 == 4) {
    ...
    void process(ProcessCallback processCallback) {
        int n2 = -1 + this.mLayoutHolderMap.size();
        while (n2 >= 0) {
