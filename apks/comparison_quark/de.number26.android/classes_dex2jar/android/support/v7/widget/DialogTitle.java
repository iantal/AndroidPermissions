  ...
  protected void onMeasure(int paramInt1, int paramInt2)
  {
    ...
    if (localLayout != null)
    {
      ...
      if ((i > 0) && (localLayout.getEllipsisCount(i - 1) > 0))
      {
        setSingleLine(false);
        setMaxLines(2);
        TypedArray localTypedArray = getContext().obtainStyledAttributes(null, a.j.TextAppearance, 16842817, 16973892);
        int j = localTypedArray.getDimensionPixelSize(a.j.TextAppearance_android_textSize, 0);
        if (j != 0) {
          setTextSize(0, j);
        }
