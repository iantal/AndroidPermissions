  ...
  implements ReprintModule
{
  ...
  private FingerprintManager.AuthenticationCallback getAuthenticationCallback(final android.support.v4.os.CancellationSignal paramCancellationSignal, final AuthenticationListener paramAuthenticationListener, final boolean paramBoolean)
  {
    new FingerprintManager.AuthenticationCallback()
    {
      ...
      {
        AuthenticationFailureReason localAuthenticationFailureReason = AuthenticationFailureReason.UNKNOWN;
        switch (paramAnonymousInt)
        {
        }
        for (;;)
        {
          paramAuthenticationListener.onFailure(localAuthenticationFailureReason, true, paramAnonymousCharSequence, 1, paramAnonymousInt);
          return;
          localAuthenticationFailureReason = AuthenticationFailureReason.HARDWARE_UNAVAILABLE;
          continue;
          localAuthenticationFailureReason = AuthenticationFailureReason.SENSOR_FAILED;
          continue;
          localAuthenticationFailureReason = AuthenticationFailureReason.TIMEOUT;
          continue;
          localAuthenticationFailureReason = AuthenticationFailureReason.LOCKED_OUT;
        }
      }
    ...
    throws SecurityException
  {
    ...
    for (android.os.CancellationSignal localCancellationSignal = null;; localCancellationSignal = (android.os.CancellationSignal)paramCancellationSignal.getCancellationSignalObject()) {
      try
      {
        localFingerprintManager.authenticate(null, localCancellationSignal, 0, localAuthenticationCallback, null);
        return;
      }
      catch (NullPointerException localNullPointerException)
      {
        this.logger.logException(localNullPointerException, "MarshmallowReprintModule: authenticate failed unexpectedly");
        paramAuthenticationListener.onFailure(AuthenticationFailureReason.UNKNOWN, true, this.context.getString(R.string.fingerprint_error_unable_to_process), 1, 5);
        return;
      }
    }
