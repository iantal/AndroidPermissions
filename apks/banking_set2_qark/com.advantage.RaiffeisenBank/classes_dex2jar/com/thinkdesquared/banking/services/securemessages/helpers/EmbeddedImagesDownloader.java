...
public class EmbeddedImagesDownloader
{
  ...
  private void populateMapOfImages()
  {
    ...
    for (int i = 0; i < this.mEmbeddedImageIds.size(); i++)
    {
    ...
    while (this.mMatcher.find()) {
      if (this.mMatcher.groupCount() == 3)
      {
        ...
        if ((this.mEmbeddedImages.containsKey(str)) && (((EmbeddedImageHolder)this.mEmbeddedImages.get(str)).status == 200))
        {
          ...
          int j = this.mMatcher.start(2) - i;
          int k = this.mMatcher.end(2) - i;
          this.mMatcher.appendReplacement(localStringBuffer, this.mMatcher.group(0).substring(0, j) + "file://" + ((EmbeddedImageHolder)this.mEmbeddedImages.get(str)).fullFilePath + this.mMatcher.group(0).substring(k, this.mMatcher.group(0).length()));
        }
      }
    }
    this.mMatcher.appendTail(localStringBuffer);
  ...
  private void setupEmbeddedImage(String paramString1, String paramString2)
  {
    ...
    {
      EmbeddedImageHolder localEmbeddedImageHolder = (EmbeddedImageHolder)this.mEmbeddedImages.get(paramString1);
      localEmbeddedImageHolder.filename = paramString2;
      localEmbeddedImageHolder.status = 400;
    }
  }
