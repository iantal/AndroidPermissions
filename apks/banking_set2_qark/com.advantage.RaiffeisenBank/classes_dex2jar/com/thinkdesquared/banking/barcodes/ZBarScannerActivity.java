  ...
  implements Camera.PreviewCallback, ZBarConstants
{
  ...
  public void onPreviewFrame(byte[] paramArrayOfByte, Camera paramCamera)
  {
    ...
    Image localImage = new Image(localSize.width, localSize.height, "Y800");
    localImage.setData(paramArrayOfByte);
    if (this.mScanner.scanImage(localImage) != 0)
    {
      this.mCamera.cancelAutoFocus();
      this.mCamera.setPreviewCallback(null);
      this.mCamera.stopPreview();
      this.mPreviewing = false;
      Iterator localIterator = this.mScanner.getResults().iterator();
      while (localIterator.hasNext())
      {
        Symbol localSymbol = (Symbol)localIterator.next();
        String str = localSymbol.getData();
        if (!TextUtils.isEmpty(str))
        {
          Intent localIntent = new Intent();
          localIntent.putExtra("SCAN_RESULT", str);
          localIntent.putExtra("SCAN_RESULT_TYPE", localSymbol.getType());
