  ...
  extends SessionBoundJob
{
    ...
    throws Throwable
  {
    ...
    String str;
    for (CreateTreasuryVerifyResponse localCreateTreasuryVerifyResponse = new DemoRequests().createTreasuryVerifyDemoRequest(this.activity, this.mTreasuryData);; localCreateTreasuryVerifyResponse = new SOAPRequests().createTreasuryVerifyRequest(this.mWorkflowId, this.mTreasuryData, str))
    {
      assertNotCancelled();
      EventBus.getDefault().post(new CreateTreasuryVerifyResponseEvent(localCreateTreasuryVerifyResponse));
      return;
