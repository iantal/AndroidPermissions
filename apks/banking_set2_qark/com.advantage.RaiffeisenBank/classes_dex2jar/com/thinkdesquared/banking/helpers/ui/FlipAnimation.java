  ...
  extends Animation
{
  ...
  {
    float f = (float)(180.0D * (3.141592653589793D * paramFloat) / 3.141592653589793D);
    if (paramFloat <= 0.05F)
    {
    ...
    this.camera.save();
    this.camera.translate(0.0F, 0.0F, 2.0F * Math.abs(f));
    this.camera.getMatrix(localMatrix);
    this.camera.rotateY(f);
    this.camera.getMatrix(localMatrix);
    this.camera.restore();
    localMatrix.preTranslate(-this.centerX, -this.centerY);
