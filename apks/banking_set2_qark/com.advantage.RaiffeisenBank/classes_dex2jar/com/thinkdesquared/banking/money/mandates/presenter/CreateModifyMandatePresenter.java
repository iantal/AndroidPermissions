  ...
  extends MvpNullObjectBasePresenter<V>
{
  ...
  public void amountClicked()
  {
    ...
    localArrayList.add(this.mData.getTransactionCurrency());
    ...
    AmountButtonEvent localAmountButtonEvent = new AmountButtonEvent(localTransactionAmountModel1, localTransactionAmountModel2, localArrayList, this.mAmountTypes, this.mData.getAmountType());
    ((CreateMandateView)getView()).amountButton(localAmountButtonEvent);
  }
  ...
  protected String getMaxAmount(String paramString, ArrayList<CurrencyLimit> paramArrayList)
  {
    ...
    if (!CollectionUtils.isEmpty(paramArrayList)) {
      for (int i = 0; i < paramArrayList.size(); i++)
      {
        ...
        if ((DSQHelper.isNotEmpty(paramString)) && (localCurrencyLimit != null) && (paramString.equalsIgnoreCase(localCurrencyLimit.getCurrency()))) {
          str = localCurrencyLimit.getLimit();
        }
      }
    }
    LogHelper.d("Currency : " + paramString + " , maxAmount : " + str);
    return str;
  }
