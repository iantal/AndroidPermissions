...
public final class TypeAdapters
{
  ...
  static
  {
    ...
    ATOMIC_INTEGER = new TypeAdapter()
    {
        ...
        throws IOException
      {
        try
        {
          AtomicInteger localAtomicInteger = new AtomicInteger(paramAnonymousJsonReader.nextInt());
  ...
  public static <T1> TypeAdapterFactory newTypeHierarchyFactory(Class<T1> paramClass, final TypeAdapter<T1> paramTypeAdapter)
  {
    new TypeAdapterFactory()
    {
      public <T2> TypeAdapter<T2> create(Gson paramAnonymousGson, TypeToken<T2> paramAnonymousTypeToken)
      {
        final Class localClass = paramAnonymousTypeToken.getRawType();
        ...
        new TypeAdapter()
        {
          public T1 read(JsonReader paramAnonymous2JsonReader)
            throws IOException
          {
            Object localObject = TypeAdapters.35.this.val$typeAdapter.read(paramAnonymous2JsonReader);
            if ((localObject != null) && (!localClass.isInstance(localObject))) {
              throw new JsonSyntaxException("Expected a " + localClass.getName() + " but was " + localObject.getClass().getName());
            }
            return localObject;
          }
