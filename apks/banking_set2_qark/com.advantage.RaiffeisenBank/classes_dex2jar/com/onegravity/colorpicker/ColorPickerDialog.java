  ...
  implements OnColorChangedListener, TabHost.OnTabChangeListener
{
  ...
  private View createExact()
  {
    ...
    this.mExactViewA.setFilters(arrayOfInputFilter);
    this.mExactViewR.setFilters(arrayOfInputFilter);
    this.mExactViewG.setFilters(arrayOfInputFilter);
    this.mExactViewB.setFilters(arrayOfInputFilter);
    EditText localEditText = this.mExactViewA;
  ...
  private void createTabs()
  {
    ...
    TabHost.TabSpec localTabSpec1 = this.mTabHost.newTabSpec("wheel").setIndicator(this.mContext.getString(R.string.color_picker_wheel)).setContent(this.mFactory);
    TabHost.TabSpec localTabSpec2 = this.mTabHost.newTabSpec("exact").setIndicator(this.mContext.getString(R.string.color_picker_exact)).setContent(this.mFactory);
    this.mTabHost.addTab(localTabSpec1);
    this.mTabHost.addTab(localTabSpec2);
    this.mTabHost.setOnTabChangedListener(this);
    if (this.mCurrentTab != null) {}
    for (String str = this.mCurrentTab;; str = "wheel")
    {
  ...
  private View createWheel()
  {
    ...
    ValueBar localValueBar = (ValueBar)localView.findViewById(R.id.valuebar);
    if (localValueBar != null) {
      this.mColorPicker.addValueBar(localValueBar);
    }
    SaturationBar localSaturationBar = (SaturationBar)localView.findViewById(R.id.saturationbar);
    if (localSaturationBar != null) {
      this.mColorPicker.addSaturationBar(localSaturationBar);
    }
    OpacityBar localOpacityBar = (OpacityBar)localView.findViewById(R.id.opacitybar);
    if (localOpacityBar != null)
    {
      if (this.mUseOpacityBar) {
        this.mColorPicker.addOpacityBar(localOpacityBar);
      }
      if (!this.mUseOpacityBar) {
        break label157;
      }
    ...
    for (int i = 0;; i = 8)
    {
      localOpacityBar.setVisibility(i);
