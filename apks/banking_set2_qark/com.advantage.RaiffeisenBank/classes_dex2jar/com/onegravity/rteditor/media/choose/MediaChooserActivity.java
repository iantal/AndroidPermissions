  ...
  implements ImageChooserManager.ImageChooserListener, AudioChooserManager.AudioChooserListener, VideoChooserManager.VideoChooserListener
{
  ...
  protected void onCreate(Bundle paramBundle)
  {
    ...
    String str;
    Constants.MediaAction localMediaAction;
    if (localBundle != null)
    {
      str = localBundle.getString(EXTRA_MEDIA_ACTION);
      if (str == null)
      {
        localMediaAction = null;
        this.mMediaAction = localMediaAction;
    ...
    do
    {
      ...
      {
        return;
        localMediaAction = Constants.MediaAction.valueOf(str);
        break;
  ...
  public void onImageChosen(final RTImage paramRTImage)
  {
    ...
    runOnUiThread(new Runnable()
    {
      public void run()
      {
        ...
        {
          String str = paramRTImage.getFilePath(RTFormat.SPANNED);
          Intent localIntent = new Intent(MediaChooserActivity.this, CropImageActivity.class).putExtra("image-source-file", str).putExtra("image-dest-file", str).putExtra("scale", true).putExtra("scaleUpIfNeeded", false).putExtra("aspectX", 0).putExtra("aspectY", 0);
          MediaChooserActivity.this.startActivityForResult(localIntent, 107);
          return;
        }
