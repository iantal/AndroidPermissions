...
class RTOperationManager
{
  ...
  {
    Stack localStack = (Stack)paramMap.get(Integer.valueOf(paramRTEditText.getId()));
    if (localStack == null)
    {
      localStack = new Stack();
      paramMap.put(Integer.valueOf(paramRTEditText.getId()), localStack);
    }
    return localStack;
  }
  ...
  void executed(RTEditText paramRTEditText, Operation paramOperation)
  {
    ...
    try
    {
      ...
      localStack2 = getRedoStack(paramRTEditText);
      while ((!localStack1.empty()) && (paramOperation.canMerge((Operation)localStack1.peek()))) {
        paramOperation.merge((Operation)localStack1.pop());
      }
      push(paramOperation, localStack1);
    }
    finally {}
    localStack2.clear();
  }
