...
public class SoapEnvelope
{
  ...
  public static boolean stringToBoolean(String paramString)
  {
    ...
    } while ((!str.equals("1")) && (!str.equals("true")));
    return true;
  }
    ...
    throws IOException, XmlPullParserException
  {
    ...
    for (Node localNode = (Node)this.bodyIn;; localNode = new Node())
    {
      localNode.parse(paramXmlPullParser);
      this.bodyIn = localNode;
      return;
    }
  }
    ...
    throws IOException, XmlPullParserException
  {
    ...
    for (int j = 0; j < localNode.getChildCount(); j++) {
      if (localNode.getElement(j) != null) {
        i++;
      }
    }
    this.headerIn = new Element[i];
    int k = 0;
    for (int m = 0; m < localNode.getChildCount(); m++)
    {
      Element localElement = localNode.getElement(m);
      if (localElement != null)
      {
        Element[] arrayOfElement = this.headerIn;
        int n = k + 1;
        arrayOfElement[k] = localElement;
        k = n;
      }
    }
  }
