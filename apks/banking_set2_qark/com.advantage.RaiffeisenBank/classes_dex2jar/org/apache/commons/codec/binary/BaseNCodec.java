  ...
  implements BinaryEncoder, BinaryDecoder
{
  ...
  private byte[] resizeBuffer(Context paramContext)
  {
    ...
    for (;;)
    {
      ...
      byte[] arrayOfByte = new byte[2 * paramContext.buffer.length];
  ...
  int readResults(byte[] paramArrayOfByte, int paramInt1, int paramInt2, Context paramContext)
  {
    if (paramContext.buffer != null)
    {
      int j = Math.min(available(paramContext), paramInt2);
      System.arraycopy(paramContext.buffer, paramContext.readPos, paramArrayOfByte, paramInt1, j);
      paramContext.readPos = (j + paramContext.readPos);
