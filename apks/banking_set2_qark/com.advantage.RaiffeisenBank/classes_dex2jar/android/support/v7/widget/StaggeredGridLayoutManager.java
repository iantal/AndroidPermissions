  ...
  extends RecyclerView.LayoutManager
{
    ...
    for (;;)
    {
      ...
      for (int n = 0; n < this.mSpanCount; n++)
      {
        ...
        if (localAnchorInfo.mOffset != Integer.MIN_VALUE) {
          this.mSpans[n].setLine(localAnchorInfo.mOffset);
        }
      }
      resolveShouldLayoutReverse();
  ...
  public View onFocusSearchFailed(View paramView, int paramInt, RecyclerView.Recycler paramRecycler, RecyclerView.State paramState)
  {
    ...
    for (int j = getLastChildPosition();; j = getFirstChildPosition())
    {
      ...
      if (!bool)
      {
        localView2 = localSpan.getFocusableViewAfter(j, i);
      ...
      for (int m = -1 + this.mSpanCount;; m--)
      {
        if (m < 0) {
          break label308;
        }
        localView2 = this.mSpans[m].getFocusableViewAfter(j, i);
        if ((localView2 != null) && (localView2 != localView1)) {
          break;
        }
      }
    ...
    for (int k = 0;; k++)
    {
      ...
      localView2 = this.mSpans[k].getFocusableViewAfter(j, i);
      if ((localView2 != null) && (localView2 != localView1)) {
        break;
      }
    }
    ...
    public View getFocusableViewAfter(int paramInt1, int paramInt2)
    {
      ...
      else
      {
        ...
        if (i >= 0)
        {
          ...
          if (localView1.isFocusable())
          {
            int j;
            if (StaggeredGridLayoutManager.this.getPosition(localView1) > paramInt1)
            {
              j = 1;
              label145:
              if (StaggeredGridLayoutManager.this.mReverseLayout) {
