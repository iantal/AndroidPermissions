...
public abstract class ResultHandler
{
  ...
  ResultHandler(Activity paramActivity, ParsedResult paramParsedResult, Result paramResult)
  {
    ...
    paramActivity.findViewById(R.id.shopper_button).setVisibility(8);
  }
  ...
  String fillInCustomSearchURL(String paramString)
  {
    ...
    if (this.rawResult != null)
    {
      ...
      paramString = str;
      if (str.contains("%t")) {
        paramString = str.replace("%t", ResultParser.parseResult(this.rawResult).getType().toString());
      }
    }
    return paramString;
  }
  ...
  final void openGoogleShopper(String paramString)
  {
    ...
    paramString = this.activity.getPackageManager().queryIntentActivities(localIntent, 65536);
    if ((paramString != null) && (!paramString.isEmpty()))
    {
      this.activity.startActivity(localIntent);
      return;
    }
    paramString = new AlertDialog.Builder(this.activity);
    paramString.setTitle(R.string.msg_google_shopper_missing);
    paramString.setMessage(R.string.msg_install_google_shopper);
    paramString.setIcon(R.drawable.shopper_icon);
    paramString.setPositiveButton(R.string.button_ok, this.shopperMarketListener);
    paramString.setNegativeButton(R.string.button_cancel, null);
    paramString.show();
  }
  ...
  {
    sendEmailFromUri("mailto:", null, this.activity.getString(R.string.msg_share_subject_line), paramString);
  }
  ...
  {
    View localView = this.activity.findViewById(R.id.shopper_button);
