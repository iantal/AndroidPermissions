  ...
  implements Scanner, Locator
{
    ...
    throws IOException, SAXException
  {
    ...
    paramScanHandler = this.theOutputBuffer;
    int i = this.theSize;
    this.theSize = (i + 1);
    paramScanHandler[i] = ((char)paramInt);
  }
    ...
    throws IOException, SAXException
  {
    ...
    paramScanHandler = new char[this.theOutputBuffer.length * 2];
    System.arraycopy(this.theOutputBuffer, 0, paramScanHandler, 0, this.theSize + 1);
    this.theOutputBuffer = paramScanHandler;
  }
    ...
    throws IOException, SAXException
  {
    ...
    for (;;)
    {
      ...
      while (paramInt < i)
      {
        ...
        int j = this.theSize;
        this.theSize = (j + 1);
        paramScanHandler[j] = paramString.charAt(paramInt);
        paramInt += 1;
      }
    }
  }
    ...
    throws IOException, SAXException
  {
    ...
    {
      paramReader = (PushbackReader)paramReader;
      i = paramReader.read();
      if (i != 65279) {
        unread(paramReader, i);
      }
      do
      {
        if (this.theState == 21) {
        ...
        this.theCurrentLine += 1;
        this.theCurrentColumn = 0;
      } while ((bool) && (i < 32) && (i != 10) && (i != 9) && (i != -1));
      j = 0;
      m = 0;
      n = j;
      if (m < statetable.length)
      {
        if (this.theState == statetable[m]) {
          break label542;
        }
        n = j;
        if (j == 0) {
    ...
    for (;;)
    {
      ...
      {
      default: 
        throw new Error("Can't process state " + n);
        if ((paramReader instanceof BufferedReader))
        {
          paramReader = new PushbackReader(paramReader);
          break label18;
        }
        paramReader = new PushbackReader(new BufferedReader(paramReader, 200));
        break label18;
        k = -1;
        break label74;
        label523:
        ...
        break label101;
        label529:
        this.theCurrentColumn += 1;
        break label222;
        label542:
        if (statetable[(m + 1)] == 0)
        {
          n = statetable[(m + 2)];
          this.theNextState = statetable[(m + 3)];
        }
        label575:
        do
        {
          ...
          n = j;
        } while (statetable[(m + 1)] != i);
        n = statetable[(m + 2)];
        this.theNextState = statetable[(m + 3)];
      }
    }
    throw new Error("HTMLScanner can't cope with " + Integer.toString(i) + " in state " + Integer.toString(this.theState));
    ...
    for (;;)
    {
      ...
      if (str != null)
      {
        ...
        {
          paramScanHandler.entity(this.theOutputBuffer, 1, this.theSize - 1);
          m = paramScanHandler.getEntity();
          if (m != 0)
          {
            this.theSize = 0;
            j = m;
          ...
          for (this.theCurrentColumn -= 1;; this.theCurrentColumn -= 1)
          {
            ...
            if (j <= 65535)
            {
              save(j, paramScanHandler);
              break label1295;
            }
            j -= 65536;
            save((j >> 10) + 55296, paramScanHandler);
            save((j & 0x3FF) + 56320, paramScanHandler);
            break label1295;
            if (bool)
            {
              unread(paramReader, k);
