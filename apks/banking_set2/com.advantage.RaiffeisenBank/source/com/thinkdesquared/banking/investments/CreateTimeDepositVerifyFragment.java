  ...
  implements LoaderManager.LoaderCallbacks<CreateTimeDepositVerifyResponse>
{
  ...
  private void initWithVerifyResponse()
  {
    ...
    if (this.mVerifyResponse.verifiedData.getIcbsRate() != null)
    {
      ...
      String str2 = this.mVerifyResponse.verifiedData.getTotalRateString(getActivity()) + " %";
      String str1 = str2;
      if (this.mVerifyResponse.verifiedData.getAibasRate() != null)
      {
        str1 = str2;
        if (this.mVerifyResponse.verifiedData.getAibasRate().compareTo(BigDecimal.ZERO) > 0) {
          str1 = str2 + " (" + getString(2131165348) + " " + this.mVerifyResponse.verifiedData.getBonusRateString(getActivity()) + "%)";
        }
      }
    ...
    extends AsyncTask<String, Void, CreateAccountResultResponse>
  {
    ...
    protected CreateAccountResultResponse doInBackground(String... paramVarArgs)
    {
      if (AibasStore.getInstance().getLoggedInState() == AibasStore.LoggedInState.LoggedInState_ON)
      {
        ...
        return new SOAPRequests().createTimeDepositResultRequest(paramVarArgs);
      }
    ...
    protected void onPostExecute(CreateAccountResultResponse paramCreateAccountResultResponse)
    {
      ...
      {
        str = CreateTimeDepositVerifyFragment.this.getString(2131166047) + " " + DSQHelper.formatIBANWithSpaces(paramCreateAccountResultResponse.getAccountNumber());
        CreateTimeDepositVerifyFragment.this.mListener.onReceivedSuccessfullResultResponse(str, true, paramCreateAccountResultResponse, 2131166253);
        return;
      }
