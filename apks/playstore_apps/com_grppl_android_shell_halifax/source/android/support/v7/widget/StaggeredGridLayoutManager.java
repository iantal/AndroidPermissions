  ...
  implements RecyclerView.SmoothScroller.ScrollVectorProvider
{
  ...
  private int fill(RecyclerView.Recycler paramRecycler, LayoutState paramLayoutState, RecyclerView.State paramState)
  {
    ...
    for (;;)
    {
      ...
      else
      {
        ...
        layoutDecoratedWithMargins(localView, j, k, i1, m);
        if (!localLayoutParams.mFullSpan) {
          break label891;
        }
      ...
      for (;;)
      {
        ...
        addView(localView, 0);
        break label190;
        j = localSpan.getEndLine(n);
        break label223;
        if (localLayoutParams.mFullSpan) {}
        for (j = getMinStart(n);; j = localSpan.getStartLine(n))
        {
          i2 = j - this.mPrimaryOrientation.getDecoratedMeasurement(localView);
          k = i2;
          m = j;
        ...
        break label371;
        if (localLayoutParams.mFullSpan) {}
        for (j = this.mSecondaryOrientation.getStartAfterPadding();; j = this.mSecondaryOrientation.getStartAfterPadding() + j * i1)
        {
        ...
        layoutDecoratedWithMargins(localView, k, j, m, i1);
        break label415;
  ...
  public View onFocusSearchFailed(View paramView, int paramInt, RecyclerView.Recycler paramRecycler, RecyclerView.State paramState)
  {
    ...
    for (paramInt = getLastChildPosition();; paramInt = getFirstChildPosition())
    {
      ...
      if (!bool)
      {
        paramRecycler = localSpan.getFocusableViewAfter(paramInt, k);
  ...
  int scrollBy(int paramInt, RecyclerView.Recycler paramRecycler, RecyclerView.State paramState)
  {
    ...
    int i;
    if (this.mLayoutState.mAvailable < j) {
      i = paramInt;
    }
    for (;;)
    {
  ...
  public void setMeasuredDimension(Rect paramRect, int paramInt1, int paramInt2)
  {
    ...
    if (this.mOrientation == 1)
    {
      paramInt2 = chooseSize(paramInt2, j + paramRect.height(), getMinimumHeight());
      paramInt1 = chooseSize(paramInt1, i + this.mSizePerSpan * this.mSpanCount, getMinimumWidth());
    }
    for (;;)
    {
      setMeasuredDimension(paramInt1, paramInt2);
      return;
      paramInt1 = chooseSize(paramInt1, i + paramRect.width(), getMinimumWidth());
      paramInt2 = chooseSize(paramInt2, j + this.mSizePerSpan * this.mSpanCount, getMinimumHeight());
    }
  }
