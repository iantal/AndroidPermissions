  ...
  implements RecyclerView.OnChildAttachStateChangeListener
{
    ...
    for (;;)
    {
      ...
      if ((localView.getBottom() >= n) && (localView.getTop() <= i2) && (localView.getRight() >= m) && (localView.getLeft() <= i1))
      {
        ...
        if (this.mCallback.canDropOver(this.mRecyclerView, this.mSelected, localViewHolder))
        {
          ...
          k = 0;
          j = 0;
          while ((k < i7) && (i6 > ((Integer)this.mDistances.get(k)).intValue()))
          {
            k += 1;
            j += 1;
          }
          this.mSwapTargets.add(j, localViewHolder);
          this.mDistances.add(j, Integer.valueOf(i6));
        }
      }
    }
    label403:
  ...
  private int swipeIfNecessary(RecyclerView.ViewHolder paramViewHolder)
  {
    ...
    do
    {
      ...
      i = checkVerticalSwipe(paramViewHolder, k);
    } while (i <= 0);
    label125:
    do
    {
  ...
  public static abstract class Callback
  {
    ...
    public RecyclerView.ViewHolder chooseDropTarget(RecyclerView.ViewHolder paramViewHolder, List<RecyclerView.ViewHolder> paramList, int paramInt1, int paramInt2)
    {
      ...
      if (k < i4)
      {
        localObject1 = (RecyclerView.ViewHolder)paramList.get(k);
        if (i2 <= 0) {
          break label374;
        }
        j = ((RecyclerView.ViewHolder)localObject1).itemView.getRight() - (paramInt1 + n);
        if ((j >= 0) || (((RecyclerView.ViewHolder)localObject1).itemView.getRight() <= paramViewHolder.itemView.getRight())) {
          break label374;
        }
        j = Math.abs(j);
        if (j <= i) {
        ...
        if (i2 < 0)
        {
          int m = ((RecyclerView.ViewHolder)localObject1).itemView.getLeft() - paramInt1;
          localObject3 = localObject2;
          i = j;
          if (m > 0)
          {
            localObject3 = localObject2;
            i = j;
            if (((RecyclerView.ViewHolder)localObject1).itemView.getLeft() < paramViewHolder.itemView.getLeft())
            {
              m = Math.abs(m);
              localObject3 = localObject2;
              i = j;
        ...
        j = ((RecyclerView.ViewHolder)localObject1).itemView.getTop() - paramInt2;
        if ((j <= 0) || (((RecyclerView.ViewHolder)localObject1).itemView.getTop() >= paramViewHolder.itemView.getTop())) {
          break label388;
        }
        j = Math.abs(j);
        if (j <= i) {
      ...
      for (;;)
      {
        if (i3 > 0)
        {
          j = ((RecyclerView.ViewHolder)localObject1).itemView.getBottom() - (paramInt2 + i1);
          if ((j < 0) && (((RecyclerView.ViewHolder)localObject1).itemView.getBottom() > paramViewHolder.itemView.getBottom()))
          {
            j = Math.abs(j);
            if (j > i) {
              i = j;
