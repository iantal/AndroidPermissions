  ...
  extends LinearLayout
{
    ...
    extends HeaderBehavior<AppBarLayout>
  {
    ...
    private void snapToChildIfNeeded(CoordinatorLayout paramCoordinatorLayout, AppBarLayout paramAppBarLayout)
    {
      ...
      int i1 = getChildIndexOnOffset(paramAppBarLayout, m);
      View localView;
      int n;
      int j;
      int k;
      ...
      if (i1 >= 0)
      {
        ...
        n = ((AppBarLayout.LayoutParams)localView.getLayoutParams()).getScrollFlags();
        if ((n & 0x11) == 17)
        {
          j = -localView.getTop();
          ...
          if (i1 == paramAppBarLayout.getChildCount() - 1) {
            i = k + paramAppBarLayout.getTopInset();
          }
          if (!checkFlag(n, 2)) {
            break label151;
          }
      ...
      for (;;)
      {
        ...
        animateOffsetTo(paramCoordinatorLayout, paramAppBarLayout, MathUtils.clamp(k, -paramAppBarLayout.getTotalScrollRange(), 0), 0.0F);
        return;
        label151:
        if (checkFlag(n, 5))
        {
          k = ViewCompat.getMinimumHeight(localView) + i;
          if (m < k)
          {
            j = i;
            i = k;
