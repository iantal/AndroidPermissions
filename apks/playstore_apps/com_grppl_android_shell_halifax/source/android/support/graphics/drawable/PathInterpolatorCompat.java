  ...
  implements Interpolator
{
  ...
  private void initPath(Path paramPath)
  {
    ...
    if ((Math.abs(this.mX[0]) > 1.0E-5D) || (Math.abs(this.mY[0]) > 1.0E-5D) || (Math.abs(this.mX[(m - 1)] - 1.0F) > 1.0E-5D) || (Math.abs(this.mY[(m - 1)] - 1.0F) > 1.0E-5D)) {
      throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.mX[0] + "," + this.mY[0] + " end:" + this.mX[(m - 1)] + "," + this.mY[(m - 1)]);
    }
    f1 = 0.0F;
    int j = 0;
    i = k;
  ...
  public float getInterpolation(float paramFloat)
  {
    ...
    f = this.mY[i];
    return paramFloat * (this.mY[j] - f) + f;
  }
}
