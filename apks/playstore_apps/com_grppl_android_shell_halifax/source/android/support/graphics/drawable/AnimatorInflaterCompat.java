...
public class AnimatorInflaterCompat
{
  ...
  private static PropertyValuesHolder getPVH(TypedArray paramTypedArray, int paramInt1, int paramInt2, int paramInt3, String paramString)
  {
    ...
    if (i == 3) {}
    for (localObject = ArgbEvaluator.getInstance();; localObject = null)
    {
      float f1;
      label349:
      float f2;
    ...
    throws XmlPullParserException, IOException
  {
    ...
    } while (!paramXmlPullParser.getName().equals("keyframe"));
    if (i == 4) {}
    for (paramInt = inferValueTypeOfKeyframe(paramResources, paramTheme, Xml.asAttributeSet(paramXmlPullParser), paramXmlPullParser);; paramInt = i)
    {
      ...
      if (localObject1 != null)
      {
        ...
        if (paramInt > 0)
        {
          ...
          paramResources = (Keyframe)localObject1.get(paramInt - 1);
  ...
  private static void setupPathMotion(Path paramPath, ObjectAnimator paramObjectAnimator, float paramFloat, String paramString1, String paramString2)
  {
    ...
    if (j < k)
    {
      ...
      f2 = ((Float)localArrayList.get(i + 1)).floatValue();
      i += 1;
