  ...
  implements Serializable
{
  ...
  {
    paramBeanDescription = paramJavaType.getRawClass();
    ClassKey localClassKey = new ClassKey(paramBeanDescription);
    if (paramBeanDescription.isInterface())
    {
      if (this._interfaceMappings == null) {
        break label159;
      }
    ...
    do
    {
      ...
      do
      {
        do
        {
          ...
          localJsonSerializer = (JsonSerializer)this._classMappings.get(localClassKey);
        ...
        paramJavaType = (JsonSerializer)this._classMappings.get(localClassKey);
        paramSerializationConfig = paramJavaType;
      } while (paramJavaType != null);
      for (paramJavaType = paramBeanDescription;; paramJavaType = paramJavaType.getSuperclass())
      {
        if (paramJavaType == null) {
          break label159;
        }
        localClassKey.reset(paramJavaType);
        localJsonSerializer = (JsonSerializer)this._classMappings.get(localClassKey);
        paramSerializationConfig = localJsonSerializer;
      ...
      paramJavaType = _findInterfaceMapping(paramBeanDescription, localClassKey);
      paramSerializationConfig = paramJavaType;
    } while (paramJavaType != null);
    if (!paramBeanDescription.isInterface())
    {
      do
      {
        paramBeanDescription = paramBeanDescription.getSuperclass();
        if (paramBeanDescription == null) {
          break;
        }
        paramSerializationConfig = _findInterfaceMapping(paramBeanDescription, localClassKey);
      } while (paramSerializationConfig == null);
      return paramSerializationConfig;
    }
    return null;
  }
