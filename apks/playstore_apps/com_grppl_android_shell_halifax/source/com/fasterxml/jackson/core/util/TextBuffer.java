...
public final class TextBuffer
{
  ...
  public int contentsToWriter(Writer paramWriter)
    throws IOException
  {
    int i;
    if (this._resultArray != null)
    {
      paramWriter.write(this._resultArray);
    ...
    do
    {
      ...
      if (this._resultString != null)
      {
        paramWriter.write(this._resultString);
    ...
    paramWriter.write(this._inputBuffer, this._inputStart, j);
    return j;
    int k;
    if (this._segments != null)
    {
      k = this._segments.size();
      j = 0;
      i = 0;
      while (j < k)
      {
        char[] arrayOfChar = (char[])this._segments.get(j);
        int m = arrayOfChar.length;
        paramWriter.write(arrayOfChar, 0, m);
        i += m;
        j += 1;
      }
    }
    for (int j = i;; j = 0)
    {
      ...
      paramWriter.write(this._currentSegment, 0, k);
      return j + k;
    }
  }
