...
public final class ErrorCorrection
{
  ...
  static
  {
    int[] arrayOfInt1 = { 83, 195, 100, 39, 188, 75, 66, 61, 241, 213, 109, 129, 94, 254, 225, 48, 90, 188 };
    int[] arrayOfInt2 = { 77, 193, 137, 31, 19, 38, 22, 153, 247, 105, 122, 2, 245, 133, 242, 8, 175, 95, 100, 9, 167, 105, 214, 111, 57, 121, 21, 1, 253, 57, 54, 101, 248, 202, 69, 50, 150, 177, 226, 5, 9, 5 };
    ...
    FACTORS = new int[][] { { 228, 48, 15, 111, 62 }, { 23, 68, 144, 134, 240, 92, 254 }, { 28, 24, 185, 166, 223, 248, 116, 255, 110, 61 }, { 175, 138, 205, 12, 194, 168, 39, 245, 60, 97, 120 }, { 41, 153, 158, 91, 61, 42, 142, 213, 97, 178, 100, 242 }, { 156, 97, 192, 252, 95, 9, 157, 119, 138, 45, 18, 186, 83, 185 }, arrayOfInt1, { 15, 195, 244, 9, 233, 71, 168, 2, 188, 160, 153, 145, 253, 79, 108, 82, 27, 174, 186, 172 }, { 52, 190, 88, 205, 109, 39, 176, 21, 155, 197, 251, 223, 155, 21, 5, 172, 254, 124, 12, 181, 184, 96, 50, 193 }, { 211, 231, 43, 97, 71, 96, 103, 174, 37, 151, 170, 53, 75, 34, 249, 121, 17, 138, 110, 213, 141, 136, 120, 151, 233, 168, 93, 255 }, { 245, 127, 242, 218, 130, 250, 162, 181, 102, 120, 84, 179, 220, 251, 80, 182, 229, 18, 2, 4, 68, 33, 101, 137, 95, 119, 115, 44, 175, 184, 59, 25, 225, 98, 81, 112 }, arrayOfInt2, { 245, 132, 172, 223, 96, 32, 117, 22, 238, 133, 238, 231, 205, 188, 237, 87, 191, 106, 16, 147, 118, 23, 37, 90, 170, 205, 131, 88, 120, 100, 66, 138, 186, 240, 82, 44, 176, 87, 187, 147, 160, 175, 69, 213, 92, 253, 225, 19 }, { 175, 9, 223, 238, 12, 17, 220, 208, 100, 29, 175, 170, 230, 192, 215, 235, 150, 159, 36, 223, 38, 200, 132, 54, 228, 146, 218, 234, 117, 203, 29, 232, 144, 238, 22, 150, 201, 117, 62, 207, 164, 13, 137, 245, 127, 67, 247, 28, 155, 43, 203, 107, 233, 53, 143, 46 }, arrayOfInt3, { 220, 228, 173, 89, 251, 149, 159, 56, 89, 33, 147, 244, 154, 36, 73, 127, 213, 136, 248, 180, 234, 197, 158, 177, 68, 122, 93, 213, 15, 160, 227, 236, 66, 139, 153, 185, 202, 167, 179, 25, 220, 232, 96, 210, 231, 136, 223, 239, 181, 241, 59, 52, 172, 25, 49, 232, 211, 189, 64, 54, 108, 153, 132, 63, 96, 103, 82, 186 } };
    LOG = new int['Ā'];
    ALOG = new int['ÿ'];
    int i = 1;
    int j = 0;
  ...
  private static String createECCBlock(CharSequence paramCharSequence, int paramInt1, int paramInt2, int paramInt3)
  {
    ...
    for (;;)
    {
      ...
      {
        int j = arrayOfChar[(paramInt3 - 1)];
        int m = paramCharSequence.charAt(i) ^ j;
        j = paramInt3 - 1;
        if (j > 0)
        {
          if ((m != 0) && (arrayOfInt[j] != 0)) {
            arrayOfChar[j] = ((char)(char)(arrayOfChar[(j - 1)] ^ ALOG[((LOG[m] + LOG[arrayOfInt[j]]) % 255)]));
          }
          for (;;)
          {
            j -= 1;
            break;
            arrayOfChar[j] = ((char)arrayOfChar[(j - 1)]);
          }
        }
        if ((m != 0) && (arrayOfInt[0] != 0)) {
          arrayOfChar[0] = ((char)(char)ALOG[((LOG[m] + LOG[arrayOfInt[0]]) % 255)]);
        }
        for (;;)
        {
          i += 1;
      ...
      {
        paramCharSequence[paramInt1] = ((char)arrayOfChar[(paramInt3 - paramInt1 - 1)]);
        paramInt1 += 1;
      }
