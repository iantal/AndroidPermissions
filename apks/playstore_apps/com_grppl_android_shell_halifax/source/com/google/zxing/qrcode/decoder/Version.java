...
public final class Version
{
  ...
  private Version(int paramInt, int[] paramArrayOfInt, ECBlocks... paramVarArgs)
  {
    ...
    while (paramInt < k)
    {
      ...
      int m = paramVarArgs.getCount();
      i += (paramVarArgs.getDataCodewords() + j) * m;
      paramInt += 1;
    }
  ...
  static Version decodeVersionInformation(int paramInt)
  {
    ...
    {
      int k = VERSION_DECODE_INFO[i];
      if (k == paramInt) {
        return getVersionForNumber(i + 7);
      }
      int n = FormatInformation.numBitsDiffering(paramInt, k);
      k = j;
      if (n < j)
      {
        m = i + 7;
