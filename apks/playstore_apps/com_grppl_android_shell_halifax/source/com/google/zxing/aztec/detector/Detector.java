...
public final class Detector
{
  ...
  private static ResultPoint[] expandSquare(ResultPoint[] paramArrayOfResultPoint, float paramFloat1, float paramFloat2)
  {
    ...
    ResultPoint localResultPoint2 = new ResultPoint(f2 - paramFloat2 * paramFloat1, f3 - f1 * paramFloat1);
    ...
    return new ResultPoint[] { localResultPoint1, new ResultPoint(paramFloat1 * paramFloat2 + f2, paramFloat1 * f1 + f3), localResultPoint2, new ResultPoint(f2 - paramFloat2 * paramFloat1, f3 - paramFloat1 * f1) };
  }
    ...
    throws NotFoundException
  {
    ...
    if (i < 4)
    {
      ...
      if (this.compact) {}
      for (l = (l << 7) + (j >> 1 & 0x7F);; l = (l << 10) + ((j >> 1 & 0x1F) + (j >> 2 & 0x3E0)))
      {
        i += 1;
        break;
      }
    ...
    throws NotFoundException
  {
    ...
    if (this.nbCenterLayers < 9)
    {
      ...
      localPoint1 = getFirstDifferent((Point)localObject2, bool, 1, 1);
      localPoint3 = getFirstDifferent((Point)localObject1, bool, -1, 1);
      localPoint2 = getFirstDifferent(paramPoint, bool, -1, -1);
      if (this.nbCenterLayers > 2)
      {
        f1 = distance(localPoint2, localPoint4) * this.nbCenterLayers / (distance(paramPoint, (Point)localObject3) * (this.nbCenterLayers + 2));
        if ((f1 >= 0.75D) && (f1 <= 1.25D) && (isWhiteOrBlackRectangle(localPoint4, localPoint1, localPoint3, localPoint2))) {}
      }
    }
    else
    {
  ...
  private Point getMatrixCenter()
  {
    try
    {
      ...
      localResultPoint3 = localObject1[0];
      localResultPoint4 = localObject1[1];
    ...
    i = MathUtils.round((localResultPoint3.getX() + ((ResultPoint)localObject1).getX() + localResultPoint4.getX() + ((ResultPoint)localObject2).getX()) / 4.0F);
    f1 = localResultPoint3.getY();
    j = MathUtils.round((((ResultPoint)localObject1).getY() + f1 + localResultPoint4.getY() + ((ResultPoint)localObject2).getY()) / 4.0F);
    try
    {
      localObject2 = new WhiteRectangleDetector(this.image, 15, i, j).detect();
      localResultPoint3 = localObject2[0];
    ...
    i = MathUtils.round((localResultPoint3.getX() + ((ResultPoint)localObject2).getX() + ((ResultPoint)localObject1).getX() + localResultPoint4.getX()) / 4.0F);
    f1 = localResultPoint3.getY();
    f2 = ((ResultPoint)localObject2).getY();
    return new Point(i, MathUtils.round((((ResultPoint)localObject1).getY() + (f1 + f2) + localResultPoint4.getY()) / 4.0F));
  }
    ...
    throws NotFoundException
  {
    ...
    } while (Integer.bitCount(EXPECTED_CORNER_BITS[k] ^ ((i & 0x1) << 11) + (i >> 1)) > 2);
    return k;
    throw NotFoundException.getNotFoundInstance();
  }
