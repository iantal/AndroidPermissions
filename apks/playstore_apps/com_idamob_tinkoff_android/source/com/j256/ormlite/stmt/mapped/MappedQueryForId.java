  ...
  extends BaseMappedQuery<T, ID>
{
    ...
    throws SQLException
  {
    ...
    if (paramDatabaseConnection == null) {
      logger.debug("{} using '{}' and {} args, got no results", this.label, this.statement, Integer.valueOf(1));
    }
    for (;;)
    {
      ...
      {
        logger.error("{} using '{}' and {} args, got >1 results", this.label, this.statement, Integer.valueOf(1));
        logArgs((Object[])localObject);
        throw new SQLException(this.label + " got more than 1 result: " + this.statement);
      }
      logger.debug("{} using '{}' and {} args, got 1 result", this.label, this.statement, Integer.valueOf(1));
    }
  }
}
