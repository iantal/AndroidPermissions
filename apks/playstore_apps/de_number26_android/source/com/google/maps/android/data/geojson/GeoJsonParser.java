...
class GeoJsonParser
{
  ...
  private ArrayList<GeoJsonFeature> parseFeatureCollection(JSONObject paramJSONObject)
  {
    ...
    for (;;)
    {
      try
      {
        ...
        if (paramJSONObject.has("bbox")) {
          this.mBoundingBox = parseBoundingBox(paramJSONObject.getJSONArray("bbox"));
        }
        i = 0;
  ...
  private void parseGeoJson()
  {
    try
    {
      Object localObject = this.mGeoJsonFile.getString("type");
      if (((String)localObject).equals("Feature"))
      {
        localObject = parseFeature(this.mGeoJsonFile);
        if (localObject == null) {
          break label116;
        }
        this.mGeoJsonFeatures.add(localObject);
        return;
      }
      ...
      {
        localObject = parseGeometryToFeature(this.mGeoJsonFile);
        if (localObject == null) {
          break label116;
        }
        this.mGeoJsonFeatures.add(localObject);
