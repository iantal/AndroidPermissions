...
public final class Encoder
{
    ...
    throws WriterException
  {
    ...
    while (i < paramInt3)
    {
      ...
      getNumDataBytesAndNumECBytesForBlockID(paramInt1, paramInt2, paramInt3, i, (int[])localObject2, (int[])localObject3);
      n = localObject2[0];
      arrayOfByte = new byte[n];
      paramBitArray.toBytes(8 * m, arrayOfByte, 0, n);
      ...
      j = Math.max(j, n);
      k = Math.max(k, localObject3.length);
      m += localObject2[0];
      i += 1;
    }
    if (paramInt2 != m) {
