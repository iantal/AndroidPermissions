    ...
    throws WriterException
  {
    ...
    while (j < m)
    {
      ...
      if ((paramArrayOfByte[j] == 63) && (c != '?'))
      {
        ...
        paramCharSequence.append("Non-encodable character detected: ");
        paramCharSequence.append(c);
        paramCharSequence.append(" (Unicode: ");
        paramCharSequence.append(c);
        paramCharSequence.append(')');
        throw new WriterException(paramCharSequence.toString());
      }
      j += 1;
    }
    return j - paramInt;
    ...
    else
    {
      ...
      while (j < i1)
      {
        ...
        else
        {
          ...
          if ((n < 5) && (m != i1))
          {
            paramCharset = paramCompaction;
            if (paramCompaction == null) {
              paramCharset = paramString.getBytes(localCharset);
            }
            n = determineConsecutiveBinaryCount(paramString, paramCharset, j);
            m = n;
            if (n == 0) {
              m = 1;
            }
            if ((m == 1) && (k == 0))
            {
              encodeBinary(paramCharset, j, 1, 0, localStringBuilder);
            }
            else
            {
              encodeBinary(paramCharset, j, m, k, localStringBuilder);
              i = 0;
              k = 1;
            }
            j += m;
            paramCompaction = paramCharset;
          }
          else
          {
            m = k;
