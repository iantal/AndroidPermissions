.class final Lcom/paypal/android/sdk/payments/ah;
.super Ljava/lang/Object;


# instance fields
.field a:Landroid/view/View;

.field b:Landroid/widget/TextView;

.field c:Landroid/widget/Button;

.field private d:Landroid/widget/ScrollView;

.field private e:Landroid/widget/LinearLayout;

.field private f:Landroid/widget/ImageView;


# direct methods
.method constructor <init>(Landroid/content/Context;Lcom/paypal/android/sdk/payments/ag;)V
    .locals 13

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    new-instance v0, Landroid/widget/ScrollView;

    invoke-direct {v0, p1}, Landroid/widget/ScrollView;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/ah;->d:Landroid/widget/ScrollView;

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/ah;->d:Landroid/widget/ScrollView;

    sget v1, Lcom/paypal/android/sdk/cy;->d:I

    invoke-virtual {v0, v1}, Landroid/widget/ScrollView;->setBackgroundColor(I)V

    new-instance v0, Landroid/widget/LinearLayout;

    invoke-direct {v0, p1}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/ah;->e:Landroid/widget/LinearLayout;

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/ah;->e:Landroid/widget/LinearLayout;

    const/4 v1, 0x1

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->setOrientation(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/ah;->d:Landroid/widget/ScrollView;

    iget-object v2, p0, Lcom/paypal/android/sdk/payments/ah;->e:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v2}, Landroid/widget/ScrollView;->addView(Landroid/view/View;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/ah;->e:Landroid/widget/LinearLayout;

    const/4 v2, -0x2

    const/4 v3, -0x1

    invoke-static {v0, v3, v2}, Lcom/paypal/android/sdk/cz;->a(Landroid/view/View;II)V

    new-instance v0, Landroid/widget/LinearLayout;

    invoke-direct {v0, p1}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->setOrientation(I)V

    const-string v4, "16dip"

    const-string v5, "20dip"

    const-string v6, "16dip"

    const-string v7, "20dip"

    invoke-static {v0, v4, v5, v6, v7}, Lcom/paypal/android/sdk/cz;->a(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    iget-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->e:Landroid/widget/LinearLayout;

    invoke-virtual {v4, v0}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    invoke-static {v0, v3, v2}, Lcom/paypal/android/sdk/cz;->a(Landroid/view/View;II)V

    new-instance v4, Landroid/widget/ImageView;

    invoke-direct {v4, p1}, Landroid/widget/ImageView;-><init>(Landroid/content/Context;)V

    iput-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->f:Landroid/widget/ImageView;

    iget-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->f:Landroid/widget/ImageView;

    const-string v5, ""

    invoke-static {v5, p1}, Lcom/paypal/android/sdk/cz;->c(Ljava/lang/String;Landroid/content/Context;)Landroid/graphics/Bitmap;

    move-result-object v5

    invoke-virtual {v4, v5}, Landroid/widget/ImageView;->setImageBitmap(Landroid/graphics/Bitmap;)V

    iget-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->f:Landroid/widget/ImageView;

    const-string v5, "PayPal"

    invoke-virtual {v4, v5}, Landroid/widget/ImageView;->setContentDescription(Ljava/lang/CharSequence;)V

    iget-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->f:Landroid/widget/ImageView;

    invoke-virtual {v0, v4}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->f:Landroid/widget/ImageView;

    const-string v5, "40dip"

    invoke-static {v4, v3, v5}, Lcom/paypal/android/sdk/cz;->a(Landroid/view/View;ILjava/lang/String;)V

    iget-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->f:Landroid/widget/ImageView;

    const-string v5, "10dip"

    const/4 v6, 0x0

    invoke-static {v4, v6, v6, v6, v5}, Lcom/paypal/android/sdk/cz;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    new-instance v4, Landroid/widget/TextView;

    invoke-direct {v4, p1}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    iput-object v4, p0, Lcom/paypal/android/sdk/payments/ah;->b:Landroid/widget/TextView;

    new-instance v4, Landroid/webkit/WebView;

    invoke-direct {v4, p1}, Landroid/webkit/WebView;-><init>(Landroid/content/Context;)V

    sget v5, Lcom/paypal/android/sdk/cy;->d:I

    invoke-virtual {v4, v5}, Landroid/webkit/WebView;->setBackgroundColor(I)V

    sget-boolean v5, Lcom/paypal/android/sdk/fu;->a:Z

    if-eqz v5, :cond_0

    const-string v5, "direction: rtl;"

    goto :goto_0

    :cond_0
    const-string v5, ""

    :goto_0
    new-instance v7, Ljava/lang/StringBuilder;

    const-string v8, "body { color: #515151; line-height: 26px; font-size: 18px; font-family: sans-serif-light, DroidSans, sans-serif; font-weight: 300; "

    invoke-direct {v7, v8}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v7, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string/jumbo v5, "} h1, strong { font-family: DroidSans, sans-serif; font-weight: 500; }h1 { font-size: 18px }h1, p { margin: 0 0 13px 0 }ul { margin: 0; padding-left: 1.1em; list-style-position: outside; }"

    invoke-virtual {v7, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v7}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v5

    new-instance v7, Ljava/lang/StringBuilder;

    const-string v8, "<html><meta name=\'viewport\' content=\'width=device-width; initial-scale=1.0;\'/><head><style type=\"text/css\">"

    invoke-direct {v7, v8}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v7, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v5, "</style></head><body>"

    invoke-virtual {v7, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    sget-object v5, Lcom/paypal/android/sdk/payments/ai;->a:[I

    invoke-virtual {p2}, Lcom/paypal/android/sdk/payments/ag;->ordinal()I

    move-result p2

    aget p2, v5, p2

    const/4 v5, 0x0

    packed-switch p2, :pswitch_data_0

    move-object p2, v6

    goto/16 :goto_2

    :pswitch_0
    sget-object p2, Lcom/paypal/android/sdk/fw;->aH:Lcom/paypal/android/sdk/fw;

    invoke-static {p2}, Lcom/paypal/android/sdk/fu;->a(Lcom/paypal/android/sdk/fw;)Ljava/lang/String;

    move-result-object p2

    new-array v8, v1, [Ljava/lang/Object;

    sget-object v9, Lcom/paypal/android/sdk/fw;->aJ:Lcom/paypal/android/sdk/fw;

    invoke-static {v9}, Lcom/paypal/android/sdk/fu;->a(Lcom/paypal/android/sdk/fw;)Ljava/lang/String;

    move-result-object v9

    aput-object v9, v8, v5

    goto :goto_1

    :pswitch_1
    sget-object p2, Lcom/paypal/android/sdk/fw;->aH:Lcom/paypal/android/sdk/fw;

    invoke-static {p2}, Lcom/paypal/android/sdk/fu;->a(Lcom/paypal/android/sdk/fw;)Ljava/lang/String;

    move-result-object p2

    new-array v8, v1, [Ljava/lang/Object;

    sget-object v9, Lcom/paypal/android/sdk/fw;->aG:Lcom/paypal/android/sdk/fw;

    invoke-static {v9}, Lcom/paypal/android/sdk/fu;->a(Lcom/paypal/android/sdk/fw;)Ljava/lang/String;

    move-result-object v9

    aput-object v9, v8, v5

    :goto_1
    invoke-static {p2, v8}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object p2

    goto :goto_2

    :pswitch_2
    sget-object p2, Lcom/paypal/android/sdk/fw;->ai:Lcom/paypal/android/sdk/fw;

    invoke-static {p2}, Lcom/paypal/android/sdk/fu;->a(Lcom/paypal/android/sdk/fw;)Ljava/lang/String;

    move-result-object p2

    goto :goto_2

    :pswitch_3
    sget-object p2, Lcom/paypal/android/sdk/fw;->am:Lcom/paypal/android/sdk/fw;

    invoke-static {p2}, Lcom/paypal/android/sdk/fu;->a(Lcom/paypal/android/sdk/fw;)Ljava/lang/String;

    move-result-object p2

    invoke-static {}, Ljava/util/Locale;->getDefault()Ljava/util/Locale;

    move-result-object v8

    invoke-virtual {v8}, Ljava/util/Locale;->getCountry()Ljava/lang/String;

    move-result-object v8

    sget-object v9, Ljava/util/Locale;->US:Ljava/util/Locale;

    invoke-virtual {v8, v9}, Ljava/lang/String;->toLowerCase(Ljava/util/Locale;)Ljava/lang/String;

    move-result-object v8

    invoke-static {v8}, Lcom/paypal/android/sdk/d;->a(Ljava/lang/CharSequence;)Z

    move-result v9

    if-eqz v9, :cond_1

    const-string/jumbo v8, "us"

    :cond_1
    new-instance v9, Ljava/util/HashSet;

    const/4 v10, 0x6

    new-array v10, v10, [Ljava/lang/String;

    const-string v11, "nl"

    aput-object v11, v10, v5

    const-string v11, "de"

    aput-object v11, v10, v1

    const/4 v11, 0x2

    const-string v12, "it"

    aput-object v12, v10, v11

    const/4 v11, 0x3

    const-string v12, "fr"

    aput-object v12, v10, v11

    const/4 v11, 0x4

    const-string v12, "gb"

    aput-object v12, v10, v11

    const/4 v11, 0x5

    const-string/jumbo v12, "us"

    aput-object v12, v10, v11

    invoke-static {v10}, Ljava/util/Arrays;->asList([Ljava/lang/Object;)Ljava/util/List;

    move-result-object v10

    invoke-direct {v9, v10}, Ljava/util/HashSet;-><init>(Ljava/util/Collection;)V

    const-string v10, ""

    invoke-interface {v9, v8}, Ljava/util/Set;->contains(Ljava/lang/Object;)Z

    move-result v9

    if-eqz v9, :cond_2

    const-string v9, "https://www.paypal.com/%s/webapps/mpp/ua/useragreement-full"

    new-array v10, v1, [Ljava/lang/Object;

    aput-object v8, v10, v5

    invoke-static {v9, v10}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v10

    :cond_2
    new-array v8, v1, [Ljava/lang/Object;

    aput-object v10, v8, v5

    goto :goto_1

    :goto_2
    invoke-virtual {v7, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string p2, "</body><html>"

    invoke-virtual {v7, p2}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v7}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p2

    const-string v5, "text/html; charset=UTF-8"

    invoke-virtual {v4, p2, v5, v6}, Landroid/webkit/WebView;->loadData(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v4, v1}, Landroid/webkit/WebView;->setClickable(Z)V

    invoke-virtual {v0, v4}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    invoke-static {v4, v3, v2}, Lcom/paypal/android/sdk/cz;->a(Landroid/view/View;II)V

    const-string p2, "10dip"

    invoke-static {v4, v6, v6, v6, p2}, Lcom/paypal/android/sdk/cz;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    new-instance p2, Landroid/widget/Button;

    invoke-direct {p2, p1}, Landroid/widget/Button;-><init>(Landroid/content/Context;)V

    iput-object p2, p0, Lcom/paypal/android/sdk/payments/ah;->c:Landroid/widget/Button;

    iget-object p2, p0, Lcom/paypal/android/sdk/payments/ah;->c:Landroid/widget/Button;

    invoke-virtual {v0, p2}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object p2, p0, Lcom/paypal/android/sdk/payments/ah;->c:Landroid/widget/Button;

    invoke-static {p2, v1, p1}, Lcom/paypal/android/sdk/cz;->a(Landroid/view/View;ZLandroid/content/Context;)V

    iget-object p1, p0, Lcom/paypal/android/sdk/payments/ah;->c:Landroid/widget/Button;

    const-string p2, "20dip"

    const-string v0, "4dip"

    invoke-static {p1, v6, p2, v6, v0}, Lcom/paypal/android/sdk/cz;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    iget-object p1, p0, Lcom/paypal/android/sdk/payments/ah;->d:Landroid/widget/ScrollView;

    iput-object p1, p0, Lcom/paypal/android/sdk/payments/ah;->a:Landroid/view/View;

    return-void

    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method
