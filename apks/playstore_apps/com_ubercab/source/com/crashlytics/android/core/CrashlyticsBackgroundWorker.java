...
class CrashlyticsBackgroundWorker
{
  ...
  Future<?> submit(final Runnable paramRunnable)
  {
    try
    {
      paramRunnable = this.executorService.submit(new Runnable()
      {
        public void run()
        {
          ...
          {
            axbe.h().e("CrashlyticsCore", "Failed to execute task.", localException);
          }
        }
      });
    ...
    axbe.h().a("CrashlyticsCore", "Executor is shut down because we're handling a fatal crash.");
    return null;
  }
  ...
  <T> T submitAndWait(Callable<T> paramCallable)
  {
    ...
    {
      axbe.h().e("CrashlyticsCore", "Failed to execute task.", paramCallable);
      return null;
      axbe.h().a("CrashlyticsCore", "Executor is shut down because we're handling a fatal crash.");
      return null;
    }
