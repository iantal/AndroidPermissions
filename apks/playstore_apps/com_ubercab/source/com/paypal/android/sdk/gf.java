package com.paypal.android.sdk;

import android.content.Context;
import android.content.res.Resources;
import android.util.TypedValue;
import android.view.View;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.ImageView.ScaleType;
import android.widget.LinearLayout;
import android.widget.LinearLayout.LayoutParams;
import android.widget.TextView;

public final class gf
{
  public ViewGroup a;
  public Button b;
  public gk c;
  public TextView d;
  public TextView e;
  public TextView f;
  public ImageView g;
  public LinearLayout h;
  public fy i;
  public LinearLayout j;
  public Button k;
  public TextView l;
  public ImageView m;
  private LinearLayout n;
  
  public gf(Context paramContext)
  {
    this.a = cz.a(paramContext);
    this.n = cz.a(this.a);
    LinearLayout localLinearLayout = cz.b(this.n);
    this.l = new TextView(paramContext);
    this.l.setText("Your Order");
    cz.a(this.l, "0dip", "0dip", "0dip", "14dip");
    this.l.setTextSize(24.0F);
    this.l.setTextColor(cy.a);
    localLinearLayout.addView(this.l, new ViewGroup.LayoutParams(-2, -2));
    this.c = new gk(paramContext, "$0.00");
    localLinearLayout.addView(this.c.b);
    cz.a(this.c.b, null, "16dip");
    this.j = cz.a(paramContext, true, 42044, localLinearLayout);
    Object localObject = new ImageView(paramContext);
    ((ImageView)localObject).setImageBitmap(cz.c("", paramContext));
    ((ImageView)localObject).setScaleType(ImageView.ScaleType.CENTER_INSIDE);
    ((ImageView)localObject).setAdjustViewBounds(true);
    ((ImageView)localObject).setContentDescription("PayPal");
    if (fu.a)
    {
      this.j.addView((View)localObject);
      cz.b((View)localObject, "4dip", "2.5dip", null, null);
      cz.a((View)localObject, -2, "28dip");
    }
    this.f = new TextView(paramContext);
    cz.a(this.f);
    this.f.setText("Pay with");
    this.j.addView(this.f);
    if (!fu.a)
    {
      this.j.addView((View)localObject);
      cz.b((View)localObject, "4dip", "2.5dip", null, null);
      cz.a((View)localObject, -2, "28dip");
    }
    localObject = new LinearLayout(paramContext);
    ((LinearLayout)localObject).setOrientation(1);
    localLinearLayout.addView((View)localObject, new LinearLayout.LayoutParams(-1, -2));
    cz.b((View)localObject, null, null, null, "16dip");
    this.d = new TextView(paramContext);
    this.d.setTextSize(14.0F);
    this.d.setTextColor(cy.l);
    this.d.setGravity(17);
    ((LinearLayout)localObject).addView(this.d, new LinearLayout.LayoutParams(-1, -2));
    cz.b(this.d, null, "8dip", null, "4dip");
    this.b = new Button(paramContext);
    ((LinearLayout)localObject).addView(this.b);
    cz.a(this.b);
    cz.a(this.b, -2, "24dip");
    cz.a(this.b, 17, 1.0F);
    this.h = cz.a(paramContext, false, 42043, localLinearLayout);
    this.m = new ImageView(paramContext);
    this.m.setAdjustViewBounds(true);
    this.m.setContentDescription("PayPal");
    this.m.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
    this.m.setImageBitmap(cz.c("", paramContext));
    this.h.addView(this.m, new ViewGroup.LayoutParams(-2, (int)TypedValue.applyDimension(1, 22.0F, paramContext.getResources().getDisplayMetrics())));
    cz.b(this.m, null, null, "6dip", "1dip");
    cz.a(this.m, -2, "22dip");
    this.m.setVisibility(8);
    this.g = new ImageView(paramContext);
    this.g.setAdjustViewBounds(true);
    this.g.setContentDescription("PayPal");
    this.g.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
    this.g.setVisibility(8);
    this.h.addView(this.g, new ViewGroup.LayoutParams(-2, (int)TypedValue.applyDimension(1, 22.0F, paramContext.getResources().getDisplayMetrics())));
    cz.b(this.g, null, null, "4dip", null);
    cz.b(this.g, "36dip", "25dip");
    this.e = new TextView(paramContext);
    cz.a(this.e);
    this.e.setText("Pay with Card");
    this.h.addView(this.e, new ViewGroup.LayoutParams(-2, -2));
    this.k = new Button(paramContext);
    this.k.setId(42042);
    cz.a(this.k);
    localLinearLayout.addView(this.k);
    cz.b(this.k, null, null, null, "16dip");
    cz.a(this.k, -2, -2);
    cz.a(this.k, 17, 1.0F);
    this.i = new fy(paramContext);
    localLinearLayout.addView(this.i.a);
    cz.a(this.i.a, -2, -2);
    cz.a(this.i.a, 17, 1.0F);
  }
}
