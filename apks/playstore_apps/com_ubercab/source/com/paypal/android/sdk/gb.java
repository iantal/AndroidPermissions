package com.paypal.android.sdk;

import java.util.Locale;
import org.json.JSONObject;

public final class gb
{
  private String a;
  private String b;
  private String c;
  private Double d;
  private String e;
  private String f;
  
  private gb(String paramString1, String paramString2, String paramString3, Double paramDouble, String paramString4, String paramString5)
  {
    this.a = paramString1;
    this.b = paramString2;
    this.c = paramString3;
    this.d = paramDouble;
    this.e = paramString4;
    this.f = paramString5;
  }
  
  public static gb a(JSONObject paramJSONObject)
  {
    Object localObject3;
    Object localObject2;
    Object localObject4;
    String str1;
    if (paramJSONObject.optJSONObject("bank_account") != null)
    {
      localObject1 = paramJSONObject.getJSONObject("bank_account");
      if (localObject1 != null)
      {
        localObject3 = ((JSONObject)localObject1).getString("bank_name");
        localObject2 = new StringBuilder();
        ((StringBuilder)localObject2).append(((JSONObject)localObject1).getString("account_type"));
        ((StringBuilder)localObject2).append(" x-");
        ((StringBuilder)localObject2).append(((JSONObject)localObject1).getString("account_number"));
        localObject4 = ((StringBuilder)localObject2).toString();
        localObject1 = fu.a(fw.e);
        localObject2 = paramJSONObject.optJSONObject("amount");
        if (localObject2 != null)
        {
          str1 = ((JSONObject)localObject2).getString("value");
          localObject1 = a(str1, ((JSONObject)localObject2).getString("currency"));
          localObject2 = Double.valueOf(Double.parseDouble(str1));
        }
        else
        {
          localObject2 = Double.valueOf(0.0D);
        }
        return new gb((String)localObject3, (String)localObject4, (String)localObject1, (Double)localObject2, "", paramJSONObject.optString("funding_mode"));
      }
      return null;
    }
    Object localObject1 = paramJSONObject.optString("funding_instrument_type");
    if ((d.b((CharSequence)localObject1)) && (((String)localObject1).equals("BALANCE")))
    {
      localObject2 = paramJSONObject.optJSONObject("amount");
      paramJSONObject = fu.a(fw.e);
      localObject1 = Double.valueOf(0.0D);
      if (localObject2 != null)
      {
        localObject1 = ((JSONObject)localObject2).getString("value");
        paramJSONObject = a((String)localObject1, ((JSONObject)localObject2).getString("currency"));
        localObject1 = Double.valueOf(Double.parseDouble((String)localObject1));
      }
      return new gb(fu.a(fw.ay), "", paramJSONObject, (Double)localObject1, "", null);
    }
    if (paramJSONObject.optJSONObject("payment_card") != null)
    {
      localObject3 = paramJSONObject.getJSONObject("payment_card");
      if (localObject3 != null)
      {
        localObject1 = ((JSONObject)localObject3).getString("type");
        if (d.a((CharSequence)localObject1)) {
          localObject1 = fw.bg;
        }
        for (;;)
        {
          localObject2 = fu.a((fw)localObject1);
          do
          {
            break label567;
            if (((String)localObject1).equals("AMERICANEXPRESS")) {}
            do
            {
              localObject1 = fw.g;
              break;
              if (((String)localObject1).equals("CARTAAURA"))
              {
                localObject1 = fw.h;
                break;
              }
              if (((String)localObject1).equals("CARTEAURORE"))
              {
                localObject1 = fw.i;
                break;
              }
            } while (((String)localObject1).equals("CARTAPREPAGATAPAYPAL"));
            if (((String)localObject1).equals("CARTEBLEUE"))
            {
              localObject1 = fw.j;
              break;
            }
            if (((String)localObject1).equals("COFINOGA"))
            {
              localObject1 = fw.l;
              break;
            }
            if (((String)localObject1).equals("DELTA"))
            {
              localObject1 = fw.m;
              break;
            }
            if (((String)localObject1).equals("DISCOVER"))
            {
              localObject1 = fw.n;
              break;
            }
            if (((String)localObject1).equals("ELECTRON"))
            {
              localObject1 = fw.o;
              break;
            }
            if (((String)localObject1).equals("JCB"))
            {
              localObject1 = fw.p;
              break;
            }
            if (((String)localObject1).equals("MAESTRO"))
            {
              localObject1 = fw.q;
              break;
            }
            if (((String)localObject1).equals("MASTERCARD"))
            {
              localObject1 = fw.r;
              break;
            }
            if (((String)localObject1).equals("POSTEPAY"))
            {
              localObject1 = fw.s;
              break;
            }
            if (((String)localObject1).equals("4ETOILES"))
            {
              localObject1 = fw.t;
              break;
            }
            if (((String)localObject1).equals("TARJETAAURORA"))
            {
              localObject1 = fw.u;
              break;
            }
            localObject2 = localObject1;
          } while (!((String)localObject1).equals("VISA"));
          localObject1 = fw.v;
        }
        label567:
        localObject1 = ((JSONObject)localObject3).getString("type");
        if (d.b((CharSequence)localObject1))
        {
          if (((String)localObject1).equals("AMERICANEXPRESS"))
          {
            localObject1 = "";
            break label660;
          }
          if (((String)localObject1).equals("DISCOVER"))
          {
            localObject1 = "";
            break label660;
          }
          if (((String)localObject1).equals("JCB"))
          {
            localObject1 = "";
            break label660;
          }
          if (((String)localObject1).equals("MASTERCARD"))
          {
            localObject1 = "";
            break label660;
          }
          if (((String)localObject1).equals("VISA"))
          {
            localObject1 = "";
            break label660;
          }
        }
        localObject1 = "";
        label660:
        localObject4 = new StringBuilder("x-");
        ((StringBuilder)localObject4).append(((JSONObject)localObject3).getString("number"));
        str1 = ((StringBuilder)localObject4).toString();
        localObject4 = paramJSONObject.optJSONObject("amount");
        localObject3 = fu.a(fw.e);
        if (localObject4 != null)
        {
          String str2 = ((JSONObject)localObject4).getString("value");
          localObject3 = a(str2, ((JSONObject)localObject4).getString("currency"));
          double d1 = Double.parseDouble(str2);
          localObject4 = Double.valueOf(d1);
        }
        else
        {
          localObject4 = Double.valueOf(0.0D);
        }
        return new gb((String)localObject2, str1, (String)localObject3, (Double)localObject4, (String)localObject1, paramJSONObject.optString("funding_mode"));
      }
      return null;
    }
    if ((paramJSONObject.optJSONObject("credit") != null) && (paramJSONObject.optJSONObject("credit") != null))
    {
      localObject1 = fu.a(fw.e);
      localObject2 = paramJSONObject.optJSONObject("amount");
      if (localObject2 != null)
      {
        localObject3 = ((JSONObject)localObject2).getString("value");
        localObject1 = a((String)localObject3, ((JSONObject)localObject2).getString("currency"));
        localObject2 = Double.valueOf(Double.parseDouble((String)localObject3));
      }
      else
      {
        localObject2 = Double.valueOf(0.0D);
      }
      paramJSONObject = paramJSONObject.optString("funding_mode");
      return new gb(fu.a(fw.az), "", (String)localObject1, (Double)localObject2, "", paramJSONObject);
    }
    return null;
  }
  
  private static String a(String paramString1, String paramString2)
  {
    de localDe = de.a();
    return ek.a(Locale.getDefault(), localDe.c().a(), Double.parseDouble(paramString1), paramString2, true);
  }
  
  public final String a()
  {
    return this.a;
  }
  
  public final String b()
  {
    return this.b;
  }
  
  public final String c()
  {
    return this.c;
  }
  
  public final Double d()
  {
    return this.d;
  }
  
  public final String e()
  {
    return this.e;
  }
  
  public final String f()
  {
    return this.f;
  }
}
