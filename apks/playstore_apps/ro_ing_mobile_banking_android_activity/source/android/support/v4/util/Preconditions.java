...
public class Preconditions
{
  ...
  public static float checkArgumentFinite(float paramFloat, String paramString)
  {
    if (Float.isNaN(paramFloat)) {
      throw new IllegalArgumentException(new StringBuilder().append(paramString).append(" must not be NaN").toString());
    }
    if (Float.isInfinite(paramFloat)) {
      throw new IllegalArgumentException(new StringBuilder().append(paramString).append(" must not be infinite").toString());
    }
    return paramFloat;
  }
  ...
  {
    checkNotNull(paramArrayOfFloat, new StringBuilder().append(paramString).append(" must not be null").toString());
    int i = 0;
    while (i < paramArrayOfFloat.length)
    {
      float f = paramArrayOfFloat[i];
      if (Float.isNaN(f)) {
        throw new IllegalArgumentException(new StringBuilder().append(paramString).append("[").append(i).append("] must not be NaN").toString());
      }
      if (f < paramFloat1) {
  ...
  public static <T> T[] checkArrayElementsNotNull(T[] paramArrayOfT, String paramString)
  {
    if (paramArrayOfT == null) {
      throw new NullPointerException(new StringBuilder().append(paramString).append(" must not be null").toString());
    }
    int i = 0;
  ...
  public static <T> Collection<T> checkCollectionNotEmpty(Collection<T> paramCollection, String paramString)
  {
    ...
    if (paramCollection.isEmpty()) {
      throw new IllegalArgumentException(new StringBuilder().append(paramString).append(" is empty").toString());
    }
  ...
  public static int checkFlagsArgument(int paramInt1, int paramInt2)
  {
    if ((paramInt1 & paramInt2) != paramInt1) {
      throw new IllegalArgumentException(new StringBuilder("Requested flags 0x").append(Integer.toHexString(paramInt1)).append(", but only 0x").append(Integer.toHexString(paramInt2)).append(" are allowed").toString());
    }
    return paramInt1;
  }
