  ...
  implements Animatable
{
  ...
  private void applyTransformation(float paramFloat, Ring paramRing, boolean paramBoolean)
  {
    ...
    {
      float f4 = paramRing.getStartingRotation();
      float f2;
      float f1;
      if (paramFloat < 0.5F)
      {
        ...
        f1 = f2;
        f2 += MATERIAL_INTERPOLATOR.getInterpolation(f3) * 0.79F + 0.01F;
      }
      else
      {
      ...
      paramRing.setRotation(f4 + 0.20999998F * paramFloat);
      setRotation(216.0F * (f3 + paramFloat));
    }
  }
