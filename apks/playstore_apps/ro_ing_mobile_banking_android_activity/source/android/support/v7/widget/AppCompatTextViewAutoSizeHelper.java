...
class AppCompatTextViewAutoSizeHelper
{
  ...
  private StaticLayout createStaticLayoutForMeasuringPre23(CharSequence paramCharSequence, Layout.Alignment paramAlignment, int paramInt)
  {
    ...
    else
    {
      ...
      bool = ((Boolean)invokeAndReturnWithDefault(this.mTextView, "getIncludeFontPadding", Boolean.TRUE)).booleanValue();
    }
  ...
  private Method getTextViewMethod(@NonNull String paramString)
  {
    ...
    {
      Log.w("ACTVAutoSizeHelper", new StringBuilder("Failed to retrieve TextView#").append(paramString).append("() method").toString(), localException);
    }
    return null;
  }
  ...
  private <T> T invokeAndReturnWithDefault(@NonNull Object paramObject, @NonNull String paramString, @NonNull T paramT)
  {
    ...
    catch (Exception paramObject)
    {
      ...
      Log.w("ACTVAutoSizeHelper", new StringBuilder("Failed to invoke TextView#").append(paramString).append("() method").toString(), paramObject);
      return paramT;
    }
    finally {}
    return paramObject;
  ...
  private boolean suggestedSizeFitsInSpace(int paramInt, RectF paramRectF)
  {
    ...
    Object localObject = (Layout.Alignment)invokeAndReturnWithDefault(this.mTextView, "getLayoutAlignment", Layout.Alignment.ALIGN_NORMAL);
    if (Build.VERSION.SDK_INT >= 23) {
      localObject = createStaticLayoutForMeasuring(localCharSequence, (Layout.Alignment)localObject, Math.round(paramRectF.right), i);
    } else {
      localObject = createStaticLayoutForMeasuringPre23(localCharSequence, (Layout.Alignment)localObject, Math.round(paramRectF.right));
    }
    if ((i != -1) && ((((Layout)localObject).getLineCount() > i) || (((Layout)localObject).getLineEnd(((Layout)localObject).getLineCount() - 1) != localCharSequence.length()))) {
      return false;
    }
    return ((Layout)localObject).getHeight() <= paramRectF.bottom;
  }
  ...
  {
    if (paramFloat1 <= 0.0F) {
      throw new IllegalArgumentException(new StringBuilder("Minimum auto-size text size (").append(paramFloat1).append("px) is less or equal to (0px)").toString());
    }
    if (paramFloat2 <= paramFloat1) {
      throw new IllegalArgumentException(new StringBuilder("Maximum auto-size text size (").append(paramFloat2).append("px) is less or equal to minimum auto-size text size (").append(paramFloat1).append("px)").toString());
    }
    if (paramFloat3 <= 0.0F) {
      throw new IllegalArgumentException(new StringBuilder("The auto-size step granularity (").append(paramFloat3).append("px) is less or equal to (0px)").toString());
    }
    this.mAutoSizeTextType = 1;
    ...
    if (supportsAutoSizeText())
    {
      ...
      if (j > 0)
      {
        ...
        if (!setupAutoSizeUniformPresetSizesConfiguration()) {
          throw new IllegalArgumentException(new StringBuilder("None of the preset sizes is valid: ").append(Arrays.toString(paramArrayOfInt)).toString());
        }
      }
      else
      {
