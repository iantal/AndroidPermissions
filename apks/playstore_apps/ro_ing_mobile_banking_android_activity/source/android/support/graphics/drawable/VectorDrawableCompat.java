  ...
  extends VectorDrawableCommon
{
  ...
  private void inflateInternal(Resources paramResources, XmlPullParser paramXmlPullParser, AttributeSet paramAttributeSet, Resources.Theme paramTheme)
  {
    ...
    if (i != 0)
    {
      ...
      throw new XmlPullParserException(new StringBuilder("no ").append(paramResources).append(" defined").toString());
    }
  }
  ...
  private void printGroupTree(VGroup paramVGroup, int paramInt)
  {
    ...
    Log.v("VectorDrawableCompat", new StringBuilder().append((String)localObject).append("current group is :").append(paramVGroup.getGroupName()).append(" rotation is ").append(paramVGroup.mRotate).toString());
    Log.v("VectorDrawableCompat", new StringBuilder().append((String)localObject).append("matrix is :").append(paramVGroup.getLocalMatrix().toString()).toString());
    i = 0;
  ...
  private void updateStateFromTypedArray(TypedArray paramTypedArray, XmlPullParser paramXmlPullParser)
  {
    ...
    if (localVPathRenderer.mViewportWidth <= 0.0F) {
      throw new XmlPullParserException(new StringBuilder().append(paramTypedArray.getPositionDescription()).append("<vector> tag requires viewportWidth > 0").toString());
    }
    if (localVPathRenderer.mViewportHeight <= 0.0F) {
      throw new XmlPullParserException(new StringBuilder().append(paramTypedArray.getPositionDescription()).append("<vector> tag requires viewportHeight > 0").toString());
    }
    ...
    if (localVPathRenderer.mBaseWidth <= 0.0F) {
      throw new XmlPullParserException(new StringBuilder().append(paramTypedArray.getPositionDescription()).append("<vector> tag requires width > 0").toString());
    }
    if (localVPathRenderer.mBaseHeight <= 0.0F) {
      throw new XmlPullParserException(new StringBuilder().append(paramTypedArray.getPositionDescription()).append("<vector> tag requires height > 0").toString());
    }
    ...
    public VGroup(VGroup paramVGroup, ArrayMap<String, Object> paramArrayMap)
    {
      ...
      while (i < localArrayList.size())
      {
        ...
        else
        {
          ...
          } else if ((paramVGroup instanceof VectorDrawableCompat.VClipPath)) {
  ...
  static class VPath
  {
    ...
    public String nodesToString(PathParser.PathDataNode[] paramArrayOfPathDataNode)
    {
      ...
      {
        str = new StringBuilder().append(str).append(paramArrayOfPathDataNode[i].mType).append(":").toString();
        float[] arrayOfFloat = paramArrayOfPathDataNode[i].mParams;
        int j = 0;
        while (j < arrayOfFloat.length)
        {
          str = new StringBuilder().append(str).append(arrayOfFloat[j]).append(",").toString();
          j += 1;
        }
        i += 1;
      }
    ...
    public void printVPath(int paramInt)
    {
      ...
      Log.v("VectorDrawableCompat", new StringBuilder().append(str).append("current path is :").append(this.mPathName).append(" pathData is ").append(nodesToString(this.mNodes)).toString());
    }
  ...
  static class VPathRenderer
  {
    ...
    private void drawPath(VectorDrawableCompat.VGroup paramVGroup, VectorDrawableCompat.VPath paramVPath, Canvas paramCanvas, int paramInt1, int paramInt2, ColorFilter paramColorFilter)
    {
      ...
      if (paramVPath.mFillColor != 0)
      {
        ...
        Paint localPaint = this.mFillPaint;
