  ...
  extends MvpBasePresenter<V>
{
  ...
  {
    long l1 = paramFileDescriptionWrapper.getFilesize();
    long l2 = l1;
    if (!CollectionUtils.isEmpty(paramList))
    {
      ...
      for (;;)
      {
        ...
        l1 += ((FileDescriptionWrapper)paramList.get(i)).getFilesize();
        i += 1;
      }
    }
    return l2;
  ...
  {
    String str2 = paramFileDescriptionWrapper.getFilepath();
    String str1 = str2;
    if (DSQHelper.isEmpty(str2)) {
      str1 = paramFileDescriptionWrapper.getFilename();
    }
    ...
    for (;;)
    {
      ...
      if (!CollectionUtils.isEmpty(paramList))
      {
        ...
        {
          paramFileDescriptionWrapper = (AllowedAttachmentFileType)paramList.get(i);
          if (FilenameUtils.getExtension(str1).equalsIgnoreCase(FileUtilsHelper.trimDotFromExtension(paramFileDescriptionWrapper.getCode()))) {
            return true;
          }
          i += 1;
        }
  ...
  protected void addSendEmailJob(String paramString1, String paramString2, String paramString3, String paramString4)
  {
    if (isViewAttached())
    {
      ...
      paramString1 = new CreateSecureMessageModel(this.files, this.mPrepareSendResponse.getWorkflowID(), paramString1, paramString2, paramString3, paramString4);
      this.mJobManager.addJobInBackground(new SendSecureMessageJob(getSessionId(), paramString1));
    }
  }
  ...
  public void validateAndAddAttachment(FileDescriptionWrapper paramFileDescriptionWrapper)
  {
    if (isViewAttached())
    {
      ...
      if (calculateFilesSize(this.files, paramFileDescriptionWrapper) > this.mPrepareSendResponse.getFileUploadMaxSize().intValue()) {
        ((SecureMessageComposeView)getView()).showValidationDialog(14, DSQHelper.getString(2131165553, new Object[] { FileUtilsHelper.readableFileSize(this.mPrepareSendResponse.getFileUploadMaxSize().intValue()) }));
      }
    }
    else
    {
