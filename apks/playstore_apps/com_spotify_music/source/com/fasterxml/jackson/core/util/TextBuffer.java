...
public final class TextBuffer
{
  ...
  public final void append(String paramString, int paramInt1, int paramInt2)
  {
    ...
    {
      i = paramInt1 + k;
      paramString.getChars(paramInt1, i, arrayOfChar, this._currentSize);
      j = paramInt2 - k;
    }
    for (;;)
    {
      expand(j);
      paramInt2 = Math.min(this._currentSegment.length, j);
      paramInt1 = i + paramInt2;
      paramString.getChars(i, paramInt1, this._currentSegment, 0);
      this._currentSize += paramInt2;
      j -= paramInt2;
      if (j <= 0) {
        return;
      ...
      else
      {
        ...
        if (i == 0)
        {
          if (j == 0) {
            ...
            localObject = new String(this._currentSegment, 0, j);
          }
          this._resultString = ((String)localObject);
        }
        else
        {
          localObject = new StringBuilder(i + j);
          if (this._segments != null)
          {
  ...
  public final String setCurrentAndReturn(int paramInt)
  {
    ...
    if (paramInt == 0) {
      ...
      str = new String(this._currentSegment, 0, paramInt);
    }
