  ...
  extends VectorDrawableCommon
{
  ...
  {
    return 0xFFFFFF & paramInt | (int)(paramFloat * Color.alpha(paramInt)) << 24;
  }
    ...
    for (;;)
    {
      ...
      else
      {
        if ("group".equals(str))
        {
          ...
          m = k;
          continue;
          if ((i == 3) && ("group".equals(paramXmlPullParser.getName())))
          {
            localStack.pop();
            m = k;
            continue;
  ...
  private static class VPathRenderer
  {
    ...
    private void drawPath(VectorDrawableCompat.VGroup paramVGroup, VectorDrawableCompat.VPath paramVPath, Canvas paramCanvas, int paramInt1, int paramInt2, ColorFilter paramColorFilter)
    {
      ...
      {
        float f5 = localVFullPath.mTrimPathStart;
        float f6 = localVFullPath.mTrimPathOffset;
        float f7 = localVFullPath.mTrimPathEnd;
        float f8 = localVFullPath.mTrimPathOffset;
        if (this.mPathMeasure == null) {
          this.mPathMeasure = new PathMeasure();
        }
        this.mPathMeasure.setPath(this.mPath, false);
        float f9 = this.mPathMeasure.getLength();
        f10 = f9 * ((f5 + f6) % 1.0F);
        f11 = f9 * ((f7 + f8) % 1.0F);
        localPath1.reset();
        if (f10 > f11)
        {
          this.mPathMeasure.getSegment(f10, f9, localPath1, true);
          this.mPathMeasure.getSegment(0.0F, f11, localPath1, true);
      ...
      for (Path.FillType localFillType = Path.FillType.WINDING;; localFillType = Path.FillType.EVEN_ODD)
      {
        ...
        localPaint1.setStrokeWidth(localVFullPath.mStrokeWidth * (f4 * f3));
        paramCanvas.drawPath(this.mRenderPath, localPaint1);
        return;
        this.mPathMeasure.getSegment(f10, f11, localPath1, true);
        break label282;
      }
    }
