  ...
  implements Animatable
{
  ...
  private void applyTransformation(float paramFloat, Ring paramRing, boolean paramBoolean)
  {
    ...
    if (paramFloat < 0.5F)
    {
      ...
      f3 = f4 + (0.01F + 0.79F * MATERIAL_INTERPOLATOR.getInterpolation(f6));
    }
    for (;;)
    {
      ...
      setRotation(216.0F * (f5 + paramFloat));
      return;
  ...
  private int evaluateColorChange(float paramFloat, int paramInt1, int paramInt2)
  {
    ...
    return i + (int)(paramFloat * ((0xFF & paramInt2 >> 24) - i)) << 24 | j + (int)(paramFloat * ((0xFF & paramInt2 >> 16) - j)) << 16 | k + (int)(paramFloat * ((0xFF & paramInt2 >> 8) - k)) << 8 | m + (int)(paramFloat * ((paramInt2 & 0xFF) - m));
  }
