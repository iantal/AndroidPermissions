  ...
  extends Lifecycle
{
  ...
  {
    Map.Entry localEntry = this.mObserverMap.ceil(paramLifecycleObserver);
    Lifecycle.State localState1;
    if (localEntry != null)
    {
      localState1 = ((ObserverWithState)localEntry.getValue()).mState;
      if (this.mParentStates.isEmpty()) {
        break label76;
      }
    }
    label76:
    for (Lifecycle.State localState2 = (Lifecycle.State)this.mParentStates.get(-1 + this.mParentStates.size());; localState2 = null)
    {
      return min(min(this.mState, localState1), localState2);
      localState1 = null;
      break;
    }
  }
  ...
  {
    this.mParentStates.remove(-1 + this.mParentStates.size());
  }
  ...
  public void addObserver(@NonNull LifecycleObserver paramLifecycleObserver)
  {
    ...
    {
      localState1 = Lifecycle.State.DESTROYED;
      localObserverWithState = new ObserverWithState(paramLifecycleObserver, localState1);
    ...
    {
      return;
      localState1 = Lifecycle.State.INITIALIZED;
      break;
    ...
    for (int i = 1;; i = 0)
    {
      Lifecycle.State localState2 = calculateTargetState(paramLifecycleObserver);
      this.mAddingObserverCounter = (1 + this.mAddingObserverCounter);
      while ((localObserverWithState.mState.compareTo(localState2) < 0) && (this.mObserverMap.contains(paramLifecycleObserver)))
      {
        pushParentState(localObserverWithState.mState);
        localObserverWithState.dispatchEvent(localLifecycleOwner, upEvent(localObserverWithState.mState));
        popParentState();
        localState2 = calculateTargetState(paramLifecycleObserver);
      }
    }
    if (i == 0) {
      sync();
