...
public class SubscriberKit
{
  ...
  {
    float f1 = this.preferredFrameRate;
    float f2 = Float.MAX_VALUE;
    if (f1 != f2)
    {
      f2 = getPreferredFramerateNative();
      if (f2 == -1.0F) {
        f2 = this.preferredFrameRate;
      }
    }
    return f2;
  }
  ...
  {
    VideoUtils.Size localSize = NO_PREFERRED_RESOLUTION;
    if (this.preferredResolution != NO_PREFERRED_RESOLUTION)
    {
      localSize = getPreferredResolutionNative();
      if (localSize == null) {
        localSize = this.preferredResolution;
      }
    }
    return localSize;
  }
