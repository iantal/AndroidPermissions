  ...
  implements Logger
{
  ...
  {
    paramMarker = new SubstituteLoggingEvent();
    paramMarker.setTimeStamp(System.currentTimeMillis());
    paramMarker.setLevel(paramLevel);
    paramMarker.setLogger(this.logger);
    paramMarker.setLoggerName(this.name);
    paramMarker.setMessage(paramString);
    paramMarker.setArgumentArray(paramArrayOfObject);
    paramMarker.setThrowable(paramThrowable);
    paramMarker.setThreadName(Thread.currentThread().getName());
    this.eventQueue.add(paramMarker);
  }
