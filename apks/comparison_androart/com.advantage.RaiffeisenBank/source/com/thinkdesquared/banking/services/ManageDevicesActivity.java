  ...
  implements ManageDevicesFragment.ManageDevicesListener, ManageDevicesRegisterDeviceVerifyFragment.ManageDevicesRegisterDeviceVerifyListener, ManageDeviceForgetVerifyFragment.ManageDeviceForgetVerifyListener, ManageDevicesChangeDeviceVerifyFragment.ManageDevicesChangeDeviceVerifyListener, ManageDevicesShowDeviceFragment.ManageDevicesChangeDeviceListener, LoadingListener, DSQInputFragment.TemplatesFragmentListener, LoaderManager.LoaderCallbacks<ManageDirectDevicesServiceResponse>
{
  ...
  {
    String str3 = paramDeviceRegistrationData.getPin();
    String str1 = CryptoUtils.generateSalt();
    String str2 = CryptoUtils.generateIV();
    str3 = CryptoUtils.encryptPIN(str3, str1);
    if (DSQHelper.isNotEmpty(paramDeviceRegistrationData.getFeedKey()))
    {
      ...
      paramDeviceRegistrationData.setFeedKey(CryptoUtils.encryptFeedKey(CryptoUtils.decryptFeedKey(paramDeviceRegistrationData.getFeedKey(), paramDeviceRegistrationData.getSalt(), paramDeviceRegistrationData.getIv()), str1, str2));
    ...
    paramDeviceRegistrationData.setSalt(str1);
    paramDeviceRegistrationData.setIv(str2);
    paramDeviceRegistrationData.setCommonToken(CryptoUtils.encryptCommonToken(str3, paramString, str2));
  }
  ...
  public void goToRegisterDevice(String paramString, boolean paramBoolean1, boolean paramBoolean2)
  {
    ...
    paramString = getSupportFragmentManager().beginTransaction();
    if ((!this.mResponse.allowedActions.contains("REMEMBER_MY_DEVICE")) || (!this.isConfigurationWidget)) {
      paramString.remove(this.fragment);
    }
    paramString.setTransition(4097);
    paramString.replace(2131558652, this.showFragment);
    if (!paramBoolean1) {
  ...
  public void onCreate(Bundle paramBundle)
  {
    ...
    if (paramBundle != null) {
      DSQHelper.setActionBarTitle(this, paramBundle, this.TAG);
    }
  ...
  public void onDeleteDevice(DeviceRegistrationData paramDeviceRegistrationData, ManageDirectDevicesDevice paramManageDirectDevicesDevice, String paramString)
  {
    ...
    for (paramDeviceRegistrationData = ManageDeviceForgetVerifyFragmentBuilder.newManageDeviceForgetVerifyFragment(paramManageDirectDevicesDevice.enrolmentId, AibasStore.getInstance().getPinLength(this), paramManageDirectDevicesDevice, paramString);; paramDeviceRegistrationData = ManageDeviceUnregisterVerifyFragmentBuilder.newManageDeviceUnregisterVerifyFragment(paramManageDirectDevicesDevice, paramString))
    {
      paramManageDirectDevicesDevice = getSupportFragmentManager().beginTransaction();
      paramManageDirectDevicesDevice.setTransition(4097);
      paramManageDirectDevicesDevice.replace(2131558652, paramDeviceRegistrationData);
      paramManageDirectDevicesDevice.addToBackStack(null);
      paramManageDirectDevicesDevice.commit();
      return;
    }
  }
  ...
  public void onLoadFinished(Loader<ManageDirectDevicesServiceResponse> paramLoader, ManageDirectDevicesServiceResponse paramManageDirectDevicesServiceResponse)
  {
    ...
    while (DSQHelper.sessionHasExpired(this, paramManageDirectDevicesServiceResponse))
    {
      do
      {
        ...
        paramLoader = findCurrentDevice();
        showListPage();
      } while (paramLoader == null);
      onDeviceSelected(this.mResponse.workflowID, paramLoader);
      return;
  ...
  {
    hideSoftwareKeyboard();
    paramRegisterDeviceRequest = ManageDevicesRegisterDeviceVerifyFragmentBuilder.newManageDevicesRegisterDeviceVerifyFragment(paramDeviceRegistrationData, this.showFragment.fingerprintLayoutDecisionsDelegate.getHasChangedAuthorizationStatus(), this.showFragment.fingerprintLayoutDecisionsDelegate.getHasChangedStatus(), paramRegisterDeviceRequest, "");
    paramDeviceRegistrationData = getSupportFragmentManager().beginTransaction();
    paramDeviceRegistrationData.setTransition(4097);
    paramDeviceRegistrationData.replace(2131558652, paramRegisterDeviceRequest);
    paramDeviceRegistrationData.addToBackStack(null);
    paramDeviceRegistrationData.commit();
  }
