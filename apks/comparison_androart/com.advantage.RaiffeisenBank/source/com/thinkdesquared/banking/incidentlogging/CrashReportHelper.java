  ...
  implements AIBASConstants
{
  ...
  {
    return getCrashReportDao().queryBuilder().orderAsc(new Property[] { CrashReportDao.Properties.Crash_date_millis }).limit(10).list();
  }
  
  public static List<CrashReport> getCrashReportsThatAreOlderThanNDays()
  {
    return getCrashReportDao().queryBuilder().orderAsc(new Property[] { CrashReportDao.Properties.Crash_date_millis }).where(CrashReportDao.Properties.Crash_date_millis.lt(Long.valueOf(System.currentTimeMillis() - 172800000L)), new WhereCondition[0]).list();
  }
  ...
  {
    paramString = new CrashReport();
    saveDefaultValuesInCrashReport(paramString, paramContext);
    paramString.setCrash_log(paramContext.getResources().getString(2131165968));
    paramString.setCrash_type("2");
    saveCrashReport(paramString);
    sendLoggingReportsToServerOrDelete(paramContext);
  }
