  ...
  implements RemoteViewsService.RemoteViewsFactory
{
  ...
  private void prepareData()
  {
    ...
    if ((this.items.size() > 0) && ((this.items.get(this.items.size() - 1) instanceof WidgetSeparatorItem))) {
  ...
  public RemoteViews getViewAt(int paramInt)
  {
    ...
    int i = 0;
    Object localObject = (WidgetItem)this.items.get(paramInt);
    if ((localObject instanceof WidgetAccountItem))
    {
      localObject = (WidgetAccountItem)this.items.get(paramInt);
      i = this.data.getWidgetAccounts().indexOf(((WidgetAccountItem)localObject).getAccount());
    }
    for (;;)
    {
      return ((WidgetItem)this.items.get(paramInt)).getView(this.context, i);
      if ((localObject instanceof WidgetTemplateItem))
      {
        localObject = (WidgetTemplateItem)this.items.get(paramInt);
        i = this.data.getWidgetTemplates().indexOf(((WidgetTemplateItem)localObject).getTemplate());
      }
    }
  }
