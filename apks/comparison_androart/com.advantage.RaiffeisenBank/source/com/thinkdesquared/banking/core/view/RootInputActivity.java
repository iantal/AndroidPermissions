  ...
  implements LoadingListener, VerifyFragment.VerifyFragmentListener, TemplateChooserFragment.OnSelectedTemplateListener, LogoContainerImageVisibilityListener, DialogInterface.OnClickListener
{
  ...
  private void lockAndDisableLogo(boolean paramBoolean)
  {
    ...
    {
      toggleLogoVisibility(paramBoolean);
      return;
    }
  }
  ...
  private void redirectToActivityInput()
  {
    ...
    Bundle localBundle = DSQHelper.prepareBundle(this);
    if (bool)
    {
      localBundle.putBoolean("INTENT_START_WITH_CONFIGURATION_WIDGET", true);
      localBundle.putInt("appWidgetId", i);
    }
  ...
  {
    paramInt = getIntent().getExtras().getInt("appWidgetId", 0);
    paramDialogInterface = new Intent(getApplication(), RZBAppWidgetConfigurationActivity.class);
    paramDialogInterface.putExtra("INTENT_CHECK_AUTHORIZED_CONTENT", true);
    paramDialogInterface.putExtra("appWidgetId", paramInt);
    overridePendingTransition(2130968594, 2130968595);
  ...
  public void onReceivedSuccessfullResultResponse(String paramString, boolean paramBoolean, GenericResultResponse paramGenericResultResponse, int paramInt)
  {
    ...
    paramBoolean = getIntent().getBooleanExtra("INTENT_START_WITH_CONFIGURATION_WIDGET", false);
    boolean bool = getIntent().getBooleanExtra("INTENT_UNREGISTER_CONFIGURATION_WIDGET", false);
    if (paramBoolean)
    {
      localBundle.putInt("appWidgetId", getIntent().getExtras().getInt("appWidgetId", 0));
      localBundle.putBoolean("INTENT_START_WITH_CONFIGURATION_WIDGET", paramBoolean);
      localBundle.putBoolean("INTENT_UNREGISTER_CONFIGURATION_WIDGET", bool);
    }
    ...
    paramString = new Intent(this, ResultActivity.class);
