  ...
  implements ServiceConnection
{
  ...
  private void hostValidator(InputStream paramInputStream, String paramString1, String paramString2)
  {
    ...
    try
    {
      ...
      paramInputStream = SSLContext.getInstance("TLS");
      paramInputStream.init(localKeyManagerFactory.getKeyManagers(), null, new SecureRandom());
      HttpsURLConnection.setDefaultSSLSocketFactory(paramInputStream.getSocketFactory());
      paramInputStream = (SSLSocket)paramInputStream.getSocketFactory().createSocket(paramString2, 443);
      paramInputStream.startHandshake();
      paramInputStream = paramInputStream.getSession().getPeerCertificates();
      paramString1 = new ArrayList();
      int i = 0;
      while (i < paramInputStream.length)
      {
        paramString1.add(paramInputStream[i]);
        i += 1;
      }
      paramInputStream = CertificateFactory.getInstance("X.509").generateCertPath(paramString1).toString();
      paramString1 = "CN=" + paramString2;
      if ((paramInputStream.contains(paramString1 + ",")) || (paramInputStream.contains(paramString1 + "\n")))
      {
