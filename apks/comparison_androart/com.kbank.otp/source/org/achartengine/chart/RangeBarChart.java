  ...
  protected void drawChartValuesText(Canvas paramCanvas, XYSeries paramXYSeries, SimpleSeriesRenderer paramSimpleSeriesRenderer, Paint paramPaint, List<Float> paramList, int paramInt1, int paramInt2)
  {
    ...
    while (i < paramList.size())
    {
      ...
      if ((!isNullValue(paramXYSeries.getY(k + 1))) && (paramList.size() > i + 3)) {
        drawText(paramCanvas, getLabel(paramSimpleSeriesRenderer.getChartValuesFormat(), paramXYSeries.getY(k + 1)), f1, ((Float)paramList.get(i + 3)).floatValue() - paramSimpleSeriesRenderer.getChartValuesSpacing(), paramPaint, 0.0F);
      }
      if ((!isNullValue(paramXYSeries.getY(k))) && (paramList.size() > i + 1)) {
        drawText(paramCanvas, getLabel(paramSimpleSeriesRenderer.getChartValuesFormat(), paramXYSeries.getY(k)), f1, ((Float)paramList.get(i + 1)).floatValue() + paramSimpleSeriesRenderer.getChartValuesTextSize() + paramSimpleSeriesRenderer.getChartValuesSpacing() - 3.0F, paramPaint, 0.0F);
      }
      i += 4;
    }
  }
