...
public class ViewDragHelper
{
  ...
  private boolean checkNewEdgeDrag(float paramFloat1, float paramFloat2, int paramInt1, int paramInt2)
  {
    ...
    return ((this.mEdgeDragsInProgress[paramInt1] & paramInt2) == 0) && (paramFloat1 > this.mTouchSlop);
  }
  ...
  private boolean checkTouchSlop(View paramView, float paramFloat1, float paramFloat2)
  {
    ...
    if (i != 0) {
      return Math.abs(paramFloat1) > this.mTouchSlop;
    }
    if (j != 0) {
  ...
  private boolean isValidPointerForActionMove(int paramInt)
  {
    ...
    {
      Log.e("ViewDragHelper", new StringBuilder("Ignoring pointerId=").append(paramInt).append(" because ACTION_DOWN was not received for this pointer before ACTION_MOVE. It likely happened because  ViewDragHelper did not receive all the events in the event stream.").toString());
      return false;
    }
    return true;
  }
  ...
  public void captureChildView(View paramView, int paramInt)
  {
    if (paramView.getParent() != this.mParentView) {
      throw new IllegalArgumentException(new StringBuilder("captureChildView: parameter must be a descendant of the ViewDragHelper's tracked parent view (").append(this.mParentView).append(")").toString());
    }
  ...
  public boolean checkTouchSlop(int paramInt1, int paramInt2)
  {
    ...
    if (i != 0) {
      return Math.abs(f1) > this.mTouchSlop;
    }
    if (paramInt1 != 0) {
