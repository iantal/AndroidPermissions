  ...
  extends Service
{
    ...
    extends AsyncTask<Void, Void, Void>
  {
    ...
    protected final void onCancelled(Void paramVoid)
    {
    ...
    extends JobIntentService.WorkEnqueuer
  {
    ...
    CompatWorkEnqueuer(Context paramContext, ComponentName paramComponentName)
    {
      ...
      this.mLaunchWakeLock = paramContext.newWakeLock(1, new StringBuilder().append(paramComponentName.getClassName()).append(":launch").toString());
      this.mLaunchWakeLock.setReferenceCounted(false);
      this.mRunWakeLock = paramContext.newWakeLock(1, new StringBuilder().append(paramComponentName.getClassName()).append(":run").toString());
      this.mRunWakeLock.setReferenceCounted(false);
    }
    
    final void enqueueWork(Intent paramIntent)
    {
    ...
    implements JobIntentService.CompatJobEngine
  {
    ...
    public final JobIntentService.GenericWorkItem dequeueWork()
    {
      synchronized (this.mLock)
      {
        ...
        localObject2 = this.mParams.dequeueWork();
      }
  ...
  static abstract class WorkEnqueuer
  {
    ...
    void ensureJobId(int paramInt)
    {
      ...
      if (this.mJobId != paramInt) {
        throw new IllegalArgumentException(new StringBuilder("Given job ID ").append(paramInt).append(" is different than previous ").append(this.mJobId).toString());
      }
    }
