  ...
  implements JsonSerializer<java.util.Date>, JsonDeserializer<java.util.Date>
{
  ...
  private java.util.Date deserializeToDate(JsonElement paramJsonElement)
  {
    for (;;)
    {
      try
      {
        ...
        {
          localDate = this.localFormat.parse(paramJsonElement.getAsString());
          return localDate;
        }
      }
      ...
      try
      {
        localDate = this.enUsFormat.parse(paramJsonElement.getAsString());
        return localDate;
      }
      ...
      try
      {
        localDate = this.iso8601Format.parse(paramJsonElement.getAsString());
        return localDate;
      }
      catch (ParseException localParseException1)
      {
        throw new JsonSyntaxException(paramJsonElement.getAsString(), localParseException1);
      }
    }
  ...
  public final java.util.Date deserialize(JsonElement paramJsonElement, Type paramType, JsonDeserializationContext paramJsonDeserializationContext)
  {
    ...
    throw new IllegalArgumentException(new StringBuilder().append(getClass()).append(" cannot deserialize to ").append(paramType).toString());
  }
