  ...
  implements Application.ActivityLifecycleCallbacks, ComponentCallbacks2
{
  ...
  public void onActivityResumed(Activity paramActivity)
  {
    if (isInBackground)
    {
      ...
      try
      {
        paramActivity = Tracker.instance();
        int i = foregroundIndex.addAndGet(1);
        if (paramActivity.getSession() != null) {
          paramActivity.getSession().setIsBackground(false);
        }
        if (paramActivity.getLifecycleEvents())
        {
          HashMap localHashMap = new HashMap();
          Util.addToMap("foregroundIndex", Integer.valueOf(i), localHashMap);
          paramActivity.track(SelfDescribing.builder().eventData(new SelfDescribingJson("iglu:com.snowplowanalytics.snowplow/application_foreground/jsonschema/1-0-0", localHashMap)).build());
          return;
        }
      }
      catch (Exception paramActivity)
      {
        Logger.e(TAG, paramActivity.getMessage(), new Object[0]);
      }
    }
  }
  ...
  public void onTrimMemory(int paramInt)
  {
    if (paramInt == 20)
    {
      ...
      try
      {
        ...
        paramInt = backgroundIndex.addAndGet(1);
        ...
        {
          HashMap localHashMap = new HashMap();
          Util.addToMap("backgroundIndex", Integer.valueOf(paramInt), localHashMap);
