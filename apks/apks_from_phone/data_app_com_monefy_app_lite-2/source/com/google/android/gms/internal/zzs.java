package com.google.android.gms.internal;

import android.os.SystemClock;
import android.util.Log;
import java.util.ArrayList;
import java.util.List;
import java.util.Locale;

public class zzs
{
  public static String a = "Volley";
  public static boolean b = Log.isLoggable(a, 2);
  
  public zzs() {}
  
  public static void a(String paramString, Object... paramVarArgs)
  {
    if (b) {
      Log.v(a, d(paramString, paramVarArgs));
    }
  }
  
  public static void a(Throwable paramThrowable, String paramString, Object... paramVarArgs)
  {
    Log.e(a, d(paramString, paramVarArgs), paramThrowable);
  }
  
  public static void b(String paramString, Object... paramVarArgs)
  {
    Log.d(a, d(paramString, paramVarArgs));
  }
  
  public static void c(String paramString, Object... paramVarArgs)
  {
    Log.e(a, d(paramString, paramVarArgs));
  }
  
  private static String d(String paramString, Object... paramVarArgs)
  {
    int i;
    label20:
    String str;
    if (paramVarArgs == null)
    {
      paramVarArgs = new Throwable().fillInStackTrace().getStackTrace();
      i = 2;
      if (i >= paramVarArgs.length) {
        break label154;
      }
      if (paramVarArgs[i].getClass().equals(zzs.class)) {
        break label147;
      }
      str = paramVarArgs[i].getClassName();
      str = str.substring(str.lastIndexOf('.') + 1);
      str = str.substring(str.lastIndexOf('$') + 1);
    }
    label147:
    label154:
    for (paramVarArgs = str + "." + paramVarArgs[i].getMethodName();; paramVarArgs = "<unknown>")
    {
      return String.format(Locale.US, "[%d] %s: %s", new Object[] { Long.valueOf(Thread.currentThread().getId()), paramVarArgs, paramString });
      paramString = String.format(Locale.US, paramString, paramVarArgs);
      break;
      i += 1;
      break label20;
    }
  }
  
  static class zza
  {
    public static final boolean a = zzs.b;
    private final List<zza> b = new ArrayList();
    private boolean c = false;
    
    zza() {}
    
    private long a()
    {
      if (this.b.size() == 0) {
        return 0L;
      }
      long l = ((zza)this.b.get(0)).c;
      return ((zza)this.b.get(this.b.size() - 1)).c - l;
    }
    
    /* Error */
    public void a(String paramString)
    {
      // Byte code:
      //   0: aload_0
      //   1: monitorenter
      //   2: aload_0
      //   3: iconst_1
      //   4: putfield 32	com/google/android/gms/internal/zzs$zza:c	Z
      //   7: aload_0
      //   8: invokespecial 49	com/google/android/gms/internal/zzs$zza:a	()J
      //   11: lstore 4
      //   13: lload 4
      //   15: lconst_0
      //   16: lcmp
      //   17: ifgt +6 -> 23
      //   20: aload_0
      //   21: monitorexit
      //   22: return
      //   23: aload_0
      //   24: getfield 30	com/google/android/gms/internal/zzs$zza:b	Ljava/util/List;
      //   27: iconst_0
      //   28: invokeinterface 43 2 0
      //   33: checkcast 9	com/google/android/gms/internal/zzs$zza$zza
      //   36: getfield 46	com/google/android/gms/internal/zzs$zza$zza:c	J
      //   39: lstore_2
      //   40: ldc 51
      //   42: iconst_2
      //   43: anewarray 4	java/lang/Object
      //   46: dup
      //   47: iconst_0
      //   48: lload 4
      //   50: invokestatic 57	java/lang/Long:valueOf	(J)Ljava/lang/Long;
      //   53: aastore
      //   54: dup
      //   55: iconst_1
      //   56: aload_1
      //   57: aastore
      //   58: invokestatic 60	com/google/android/gms/internal/zzs:b	(Ljava/lang/String;[Ljava/lang/Object;)V
      //   61: aload_0
      //   62: getfield 30	com/google/android/gms/internal/zzs$zza:b	Ljava/util/List;
      //   65: invokeinterface 64 1 0
      //   70: astore_1
      //   71: aload_1
      //   72: invokeinterface 70 1 0
      //   77: ifeq -57 -> 20
      //   80: aload_1
      //   81: invokeinterface 74 1 0
      //   86: checkcast 9	com/google/android/gms/internal/zzs$zza$zza
      //   89: astore 6
      //   91: aload 6
      //   93: getfield 46	com/google/android/gms/internal/zzs$zza$zza:c	J
      //   96: lstore 4
      //   98: ldc 76
      //   100: iconst_3
      //   101: anewarray 4	java/lang/Object
      //   104: dup
      //   105: iconst_0
      //   106: lload 4
      //   108: lload_2
      //   109: lsub
      //   110: invokestatic 57	java/lang/Long:valueOf	(J)Ljava/lang/Long;
      //   113: aastore
      //   114: dup
      //   115: iconst_1
      //   116: aload 6
      //   118: getfield 78	com/google/android/gms/internal/zzs$zza$zza:b	J
      //   121: invokestatic 57	java/lang/Long:valueOf	(J)Ljava/lang/Long;
      //   124: aastore
      //   125: dup
      //   126: iconst_2
      //   127: aload 6
      //   129: getfield 81	com/google/android/gms/internal/zzs$zza$zza:a	Ljava/lang/String;
      //   132: aastore
      //   133: invokestatic 60	com/google/android/gms/internal/zzs:b	(Ljava/lang/String;[Ljava/lang/Object;)V
      //   136: lload 4
      //   138: lstore_2
      //   139: goto -68 -> 71
      //   142: astore_1
      //   143: aload_0
      //   144: monitorexit
      //   145: aload_1
      //   146: athrow
      // Local variable table:
      //   start	length	slot	name	signature
      //   0	147	0	this	zza
      //   0	147	1	paramString	String
      //   39	100	2	l1	long
      //   11	126	4	l2	long
      //   89	39	6	localZza	zza
      // Exception table:
      //   from	to	target	type
      //   2	13	142	finally
      //   23	71	142	finally
      //   71	136	142	finally
    }
    
    public void a(String paramString, long paramLong)
    {
      try
      {
        if (this.c) {
          throw new IllegalStateException("Marker added to finished log");
        }
      }
      finally {}
      this.b.add(new zza(paramString, paramLong, SystemClock.elapsedRealtime()));
    }
    
    protected void finalize()
    {
      if (!this.c)
      {
        a("Request on the loose");
        zzs.c("Marker log finalized without finish() - uncaught exit point for request", new Object[0]);
      }
    }
    
    private static class zza
    {
      public final String a;
      public final long b;
      public final long c;
      
      public zza(String paramString, long paramLong1, long paramLong2)
      {
        this.a = paramString;
        this.b = paramLong1;
        this.c = paramLong2;
      }
    }
  }
}
