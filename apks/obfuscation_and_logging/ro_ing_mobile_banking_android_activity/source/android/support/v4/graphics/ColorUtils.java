...
public final class ColorUtils
{
  ...
  public static void RGBToHSL(@IntRange(from=0L, to=255L) int paramInt1, @IntRange(from=0L, to=255L) int paramInt2, @IntRange(from=0L, to=255L) int paramInt3, @NonNull float[] paramArrayOfFloat)
  {
    ...
    float f6 = Math.max(f1, Math.max(f3, f4));
    float f7 = Math.min(f1, Math.min(f3, f4));
    float f2 = f6 - f7;
    float f5 = (f6 + f7) / 2.0F;
    if (f6 == f7)
    {
    ...
    paramArrayOfFloat[0] = constrain(f3, 0.0F, 360.0F);
    paramArrayOfFloat[1] = constrain(f2, 0.0F, 1.0F);
    paramArrayOfFloat[2] = constrain(f5, 0.0F, 1.0F);
  }
  ...
  public static double calculateContrast(@ColorInt int paramInt1, @ColorInt int paramInt2)
  {
    if (Color.alpha(paramInt2) != 255) {
      throw new IllegalArgumentException(new StringBuilder("background can not be translucent: #").append(Integer.toHexString(paramInt2)).toString());
    }
    int i = paramInt1;
