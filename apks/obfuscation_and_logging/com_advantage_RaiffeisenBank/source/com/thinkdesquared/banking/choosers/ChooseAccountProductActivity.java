  ...
  extends AppCompatActivity
{
  ...
  private void setContent()
  {
    ...
    this.mStickyListView.setOnItemClickListener(new AdapterView.OnItemClickListener()
    {
      ...
      {
        paramAnonymousView = (AccountProduct)ChooseAccountProductActivity.this.mProducts.get(paramAnonymousInt);
        paramAnonymousAdapterView = new Bundle();
        paramAnonymousAdapterView.putString("code", paramAnonymousView.getProductCode());
        paramAnonymousView = new Intent();
        paramAnonymousView.putExtras(paramAnonymousAdapterView);
        ChooseAccountProductActivity.this.setResult(-1, paramAnonymousView);
  ...
  public void onCreate(Bundle paramBundle)
  {
    ...
    paramBundle = ChooseAccountProductSingleton.getInstance();
    this.mOffers = paramBundle.getAccountOffers();
    paramBundle.setAccountOffers(null);
    ...
    extends StickyListHeadersBaseAdapter
  {
    ...
    private void populateCellWithImage(AccountProduct paramAccountProduct, ImageView paramImageView)
    {
      if (paramAccountProduct.getTypeRes() == 2131166218) {
        paramAccountProduct = this.mIconCurrentAccount;
      }
      for (;;)
      {
        paramImageView.setImageBitmap(paramAccountProduct);
        return;
        if (paramAccountProduct.getTypeRes() == 2131166220) {
          paramAccountProduct = this.mIconSavingsAccount;
        } else if (paramAccountProduct.getTypeRes() == 2131166221) {
          paramAccountProduct = this.mIconTimeAccount;
        } else if (paramAccountProduct.getTypeRes() == 2131166219) {
          paramAccountProduct = this.mIconLoan;
        } else {
          paramAccountProduct = this.mIconCreditCard;
        }
      }
    }
