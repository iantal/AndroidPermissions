  ...
  extends AppCompatActivity
{
  ...
  private void companyWasSelected(Company paramCompany, boolean paramBoolean)
  {
    ...
    paramCompany = new Intent();
    paramCompany.putExtras(localBundle);
    DSQHelper.hideSoftwareKeyboard(this);
    setResult(-1, paramCompany);
    finish();
  }
  ...
  private ArrayList<Company> searchCompanyArrayList(String paramString)
  {
    ...
    {
      localObject = this.mCompaniesList;
      return localObject;
    }
    int i = 0;
    for (;;)
    {
      ...
      if (i >= this.mCompaniesList.size()) {
        break;
      }
      localObject = ((Company)this.mCompaniesList.get(i)).companyName;
      String str = ((Company)this.mCompaniesList.get(i)).accountNumber;
      if ((((String)localObject).toLowerCase().contains(paramString.toLowerCase())) || (str.toLowerCase().contains(paramString.toLowerCase()))) {
  ...
  private void setContent()
  {
    ...
    this.mListView.setOnItemClickListener(new AdapterView.OnItemClickListener()
    {
      ...
      {
        paramAnonymousAdapterView = (Company)ChooseCompanyActivity.this.mCompaniesList.get(paramAnonymousInt);
        ChooseCompanyActivity.this.companyWasSelected(paramAnonymousAdapterView, false);
      }
    });
  }
  ...
  private void updateCompanyListAfterSearch(final String paramString)
  {
    ...
    this.mListView.setOnItemClickListener(new AdapterView.OnItemClickListener()
    {
      public void onItemClick(AdapterView<?> paramAnonymousAdapterView, View paramAnonymousView, int paramAnonymousInt, long paramAnonymousLong)
      {
        paramAnonymousAdapterView = (Company)ChooseCompanyActivity.this.searchCompanyArrayList(paramString).get(paramAnonymousInt);
        if (!ChooseCompanyActivity.isCompanyAllowed(ChooseCompanyActivity.this.mSelectedAccount, paramAnonymousAdapterView))
        {
  ...
  public void onCreate(Bundle paramBundle)
  {
    ...
    paramBundle = new DSQBitmap(this).paintDrawableRes(2130838081, DSQStylist.fetchThemedResource(this, 2130772073));
    this.mSearchField.setCompoundDrawablesWithIntrinsicBounds(paramBundle, null, null, null);
    paramBundle = ChooseCompanySingleton.getInstance();
    this.mCompaniesList = paramBundle.getCompanies();
    ...
    ((ViewGroup)findViewById(2131558607)).setOnClickListener(new View.OnClickListener()
    {
      public void onClick(View paramAnonymousView)
      {
        ...
        ChooseCompanyActivity.this.setResult(-1, paramAnonymousView);
    ...
    extends AutoFilteringSpinnerAdapter<Company>
  {
    ...
    public View getView(final int paramInt, View paramView, ViewGroup paramViewGroup)
    {
      ...
      if (paramView == null)
      {
        ...
        localView = this.mInflater.inflate(this.cellResourceId, paramViewGroup, false);
        localViewHolder.companyNameTextView = ((TextView)localView.findViewById(2131558740));
        localViewHolder.accountNumberTextView = ((TextView)localView.findViewById(2131558741));
        localViewHolder.currencyTextView = ((TextView)localView.findViewById(2131558743));
        localViewHolder.barCodeImageView = ((ImageView)localView.findViewById(2131558742));
        localViewHolder.selectedImage = ((ImageView)localView.findViewById(2131558739));
        localView.setTag(localViewHolder);
        initRow(paramInt, localView, localViewHolder);
        localCompany = (Company)this.companyList.get(paramInt);
        String str = localCompany.companyName;
        ...
        localViewHolder.accountNumberTextView.setText(paramView);
        if (!ChooseCompanyActivity.this.mSelectedCompanyName.equals(str)) {
          break label338;
        }
        ...
        localViewHolder.barCodeImageView.setOnClickListener(new View.OnClickListener()
        {
          public void onClick(View paramAnonymousView)
          {
            ...
            do
            {
              ...
              paramAnonymousView = (Company)ChooseCompanyActivity.CompaniesSearchAdapter.this.companyList.get(paramInt);
            } while (!ChooseCompanyActivity.CompaniesSearchAdapter.this.checkForError(paramAnonymousView));
            ChooseCompanyActivity.this.companyWasSelected(paramAnonymousView, true);
          }
        });
        if (this.mCurrencyFromRowMode)
        {
          paramView = localCompany.getBeneficiaryCurrency();
          if (paramView != null) {
            localViewHolder.currencyTextView.setText((CharSequence)DefaultValueWrapper.getValue(paramView.getCode(), ""));
          }
        }
        applyAutofilteringRules(paramViewGroup, localView, localViewHolder, localCompany);
        return localView;
    ...
    {
      paramAutoFilteringSpinnerAdapter = (ViewHolder)paramAutoFilteringSpinnerAdapter;
      DSQHelper.setAlpha(paramAutoFilteringSpinnerAdapter.companyNameTextView, paramFloat);
      DSQHelper.setAlpha(paramAutoFilteringSpinnerAdapter.accountNumberTextView, paramFloat);
      DSQHelper.setAlpha(paramAutoFilteringSpinnerAdapter.barCodeImageView, paramFloat);
    }
