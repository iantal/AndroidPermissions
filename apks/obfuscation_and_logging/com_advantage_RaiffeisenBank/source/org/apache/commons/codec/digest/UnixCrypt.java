...
public class UnixCrypt
{
  ...
  private static final boolean[] SHIFT2 = { 0, 0, 1, 1, 1, 1, 1, 1, 0, 1, 1, 1, 1, 1, 1, 0 };
  ...
  private static int[] body(int[] paramArrayOfInt, int paramInt1, int paramInt2)
  {
    ...
    while (k < 25)
    {
      ...
      while (m < 32)
      {
        i = dEncrypt(i, j, m, paramInt1, paramInt2, paramArrayOfInt);
        j = dEncrypt(j, i, m + 2, paramInt1, paramInt2, paramArrayOfInt);
        m += 4;
      }
      m = i;
      k += 1;
    ...
    permOp(i >>> 1 | i << 31, j >>> 1 | j << 31, 1, 1431655765, paramArrayOfInt);
    paramInt1 = paramArrayOfInt[0];
    permOp(paramArrayOfInt[1], paramInt1, 8, 16711935, paramArrayOfInt);
    paramInt1 = paramArrayOfInt[0];
    permOp(paramArrayOfInt[1], paramInt1, 2, 858993459, paramArrayOfInt);
    paramInt1 = paramArrayOfInt[0];
    permOp(paramArrayOfInt[1], paramInt1, 16, 65535, paramArrayOfInt);
    paramInt1 = paramArrayOfInt[0];
    permOp(paramArrayOfInt[1], paramInt1, 4, 252645135, paramArrayOfInt);
    paramInt1 = paramArrayOfInt[0];
    return new int[] { paramArrayOfInt[1], paramInt1 };
  }
  
  private static int byteToUnsigned(byte paramByte)
  {
    int i = paramByte;
    if (paramByte < 0) {
      i = paramByte + 256;
  ...
  {
    return crypt(paramString.getBytes(Charsets.UTF_8));
  }
  ...
  public static String crypt(byte[] paramArrayOfByte, String paramString)
  {
    if (paramString == null)
    {
      ...
      i = SALT_CHARS.length;
      localObject = "" + SALT_CHARS[paramString.nextInt(i)] + SALT_CHARS[paramString.nextInt(i)];
    }
    do
    {
      paramString = new StringBuilder("             ");
      char c1 = ((String)localObject).charAt(0);
      char c2 = ((String)localObject).charAt(1);
      paramString.setCharAt(0, c1);
      paramString.setCharAt(1, c2);
      j = CON_SALT[c1];
      k = CON_SALT[c2];
    ...
    } while (paramString.matches("^[./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz]{2,}$"));
    throw new IllegalArgumentException("Invalid salt value: " + paramString);
    int i = 0;
    while ((i < localObject.length) && (i < paramArrayOfByte.length))
    {
      localObject[i] = ((byte)(paramArrayOfByte[i] << 1));
      i += 1;
    }
    paramArrayOfByte = body(desSetKey((byte[])localObject), j, k << 4);
    Object localObject = new byte[9];
    intToFourBytes(paramArrayOfByte[0], (byte[])localObject, 0);
    intToFourBytes(paramArrayOfByte[1], (byte[])localObject, 4);
    localObject[8] = 0;
    int k = 2;
    int m = 0;
    int j = 128;
    while (k < 13)
    {
      ...
      while (n < 6)
      {
        ...
        paramString.setCharAt(k, (char)COV2CHAR[i]);
        n += 1;
        m = i1;
      }
      k += 1;
  ...
  private static int dEncrypt(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int paramInt5, int[] paramArrayOfInt)
  {
    int i = paramInt2 ^ paramInt2 >>> 16;
    paramInt4 = i & paramInt4;
    paramInt5 = i & paramInt5;
    paramInt4 = paramInt4 << 16 ^ paramInt4 ^ paramInt2 ^ paramArrayOfInt[paramInt3];
    paramInt2 = paramInt5 << 16 ^ paramInt5 ^ paramInt2 ^ paramArrayOfInt[(paramInt3 + 1)];
    paramInt2 = paramInt2 >>> 4 | paramInt2 << 28;
    return paramInt1 ^ (SPTRANS[1][(paramInt2 & 0x3F)] | SPTRANS[3][(paramInt2 >>> 8 & 0x3F)] | SPTRANS[5][(paramInt2 >>> 16 & 0x3F)] | SPTRANS[7][(paramInt2 >>> 24 & 0x3F)] | SPTRANS[0][(paramInt4 & 0x3F)] | SPTRANS[2][(paramInt4 >>> 8 & 0x3F)] | SPTRANS[4][(paramInt4 >>> 16 & 0x3F)] | SPTRANS[6][(paramInt4 >>> 24 & 0x3F)]);
  }
  
  private static int[] desSetKey(byte[] paramArrayOfByte)
  {
    int[] arrayOfInt = new int[32];
    int i = fourBytesToInt(paramArrayOfByte, 0);
    int j = fourBytesToInt(paramArrayOfByte, 4);
    paramArrayOfByte = new int[2];
    permOp(j, i, 4, 252645135, paramArrayOfByte);
    i = paramArrayOfByte[0];
    j = hPermOp(paramArrayOfByte[1], -2, -859045888);
    permOp(hPermOp(i, -2, -859045888), j, 1, 1431655765, paramArrayOfByte);
    i = paramArrayOfByte[0];
    permOp(paramArrayOfByte[1], i, 8, 16711935, paramArrayOfByte);
    i = paramArrayOfByte[0];
    permOp(paramArrayOfByte[1], i, 1, 1431655765, paramArrayOfByte);
    ...
    if (i < 16)
    {
      ...
      for (k = k >>> 2 | k << 26;; k = k >>> 1 | k << 27)
      {
        m &= 0xFFFFFFF;
        k &= 0xFFFFFFF;
        int i1 = SKB[0][(m & 0x3F)] | SKB[1][(m >>> 6 & 0x3 | m >>> 7 & 0x3C)] | SKB[2][(m >>> 13 & 0xF | m >>> 14 & 0x30)] | SKB[3][(m >>> 20 & 0x1 | m >>> 21 & 0x6 | m >>> 22 & 0x38)];
        int i2 = SKB[4][(k & 0x3F)] | SKB[5][(k >>> 7 & 0x3 | k >>> 8 & 0x3C)] | SKB[6][(k >>> 15 & 0x3F)] | SKB[7][(k >>> 21 & 0xF | k >>> 22 & 0x30)];
        int n = j + 1;
        arrayOfInt[j] = (i2 << 16 | 0xFFFF & i1);
        i1 = i1 >>> 16 | 0xFFFF0000 & i2;
        j = n + 1;
        arrayOfInt[n] = (i1 << 4 | i1 >>> 28);
        i += 1;
        break;
        m = m >>> 1 | m << 27;
      }
  ...
  {
    int j = paramInt + 1;
    paramInt = byteToUnsigned(paramArrayOfByte[paramInt]);
    int i = j + 1;
    j = byteToUnsigned(paramArrayOfByte[j]);
    int k = i + 1;
    return paramInt | j << 8 | byteToUnsigned(paramArrayOfByte[i]) << 16 | byteToUnsigned(paramArrayOfByte[k]) << 24;
  }
  
  private static int hPermOp(int paramInt1, int paramInt2, int paramInt3)
  {
    paramInt3 = (paramInt1 << 16 - paramInt2 ^ paramInt1) & paramInt3;
    return paramInt1 ^ paramInt3 ^ paramInt3 >>> 16 - paramInt2;
  }
  
  private static void intToFourBytes(int paramInt1, byte[] paramArrayOfByte, int paramInt2)
  {
    int i = paramInt2 + 1;
    ...
    paramArrayOfByte[i] = ((byte)(paramInt1 >>> 24 & 0xFF));
  }
  
  private static void permOp(int paramInt1, int paramInt2, int paramInt3, int paramInt4, int[] paramArrayOfInt)
  {
    paramInt4 = (paramInt1 >>> paramInt3 ^ paramInt2) & paramInt4;
    paramArrayOfInt[0] = (paramInt1 ^ paramInt4 << paramInt3);
    paramArrayOfInt[1] = (paramInt2 ^ paramInt4);
  }
}
